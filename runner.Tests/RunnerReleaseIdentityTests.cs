...
public sealed class RunnerReleaseIdentityTests
{
    ...
    public void Current_symlink_target_name_is_the_advertised_release()
    {
        ...
        try
        {
            ...
            Assert.Equal(
                releaseId,
                RunnerReleaseIdentity.Resolve(current + Path.DirectorySeparatorChar, configured: ""));
            Assert.Equal(
                releaseId,
                RunnerReleaseIdentity.Resolve(release + Path.DirectorySeparatorChar, configured: ""));
        }
        finally
        {
