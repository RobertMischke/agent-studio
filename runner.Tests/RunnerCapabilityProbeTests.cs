...
public sealed class RunnerCapabilityProbeTests
{
    ...
    [InlineData(1, "", "login required", false)]
    [InlineData(1, "", "Claude authentication failed: login required", false)]
    ...
    [Fact]
    public void Provider_named_auth_line_is_distinguishable_from_tool_output()
        => Assert.True(RunnerCapabilityProbe.IsProviderAuthenticationFailure(
            new ProcessResult(1, "", "Claude authentication failed: login required"),
            "claude"));
