...
public static partial class ProviderFailureClassifier
{
    ...
    private static readonly string[] UnambiguousProviderAccountSignals =
    [
        "chatgpt account authentication failed",
    ];
        ...
        string? stderr)
    {
        ...
        if (ContainsAny(text, TransientSignals) && AuthContextRegex().IsMatch(text))
        ...
        bool trustedProviderStatus = false)
    {
        var relevant = string.Join('\n', Lines(text)
            .Where(line => !LooksLikeGitAuthentication(line) && !LooksLikeToolFailure(line)));
        if (string.IsNullOrWhiteSpace(relevant)) return false;
        if (trustedProviderStatus
            && (ContainsAny(relevant, SignedOutSignals) || UnauthorizedRegex().IsMatch(relevant)))
            return true;

        // Run stderr can contain arbitrary tool output. Generic phrases such as
        // "login required", "invalid API key", or a bare 401 are therefore not
        // provider-auth evidence. Accept them only in a provider-specific line;
        // the account-level Codex signature is unambiguous on its own.
        foreach (var line in Lines(relevant))
        {
            if (ContainsAny(line, UnambiguousProviderAccountSignals)) return true;
            if (string.IsNullOrWhiteSpace(provider)) continue;
            if (provider.Equals("codex", StringComparison.OrdinalIgnoreCase)
                && ContainsAny(line, ProviderProcessSignedOutSignals))
                return true;
            if (line.Contains(provider, StringComparison.OrdinalIgnoreCase)
                && (ContainsAny(line, SignedOutSignals)
                    || UnauthorizedRegex().IsMatch(line) && LoginMaterialRegex().IsMatch(line)))
                return true;
        }
        return false;
    }
    ...
    [GeneratedRegex(@"\b(?:auth(?:entication)?|bearer|credentials?|login|oauth|access token|refresh token|token refresh|api key)\b", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant)]
