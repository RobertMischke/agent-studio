...
public sealed class ProviderAuthProbe
{
    ...
    {
        if (status.Status != Ready || !credential.MetadataAvailable) return status;
        return status with
        {
            Detail = credential.Warning
                ? $"{credential.Detail} {status.Detail}"
                : status.Detail,
            OperationalState = credential.Warning
                ? ProviderAuthOperationalStates.CredentialsExpiring
                : status.OperationalState,
            ExpiresAt = credential.ExpiresAt ?? status.ExpiresAt,
            CredentialLastModifiedAt = credential.LastModifiedAt ?? status.CredentialLastModifiedAt,
        };
    }
