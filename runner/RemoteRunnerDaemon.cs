...
public sealed class RemoteRunnerDaemon
{
    ...
    private readonly ProviderAuthLauncher? _providerProbeLauncher;

    public RemoteRunnerDaemon(
        RunnerOptions options,
        TaskServerClient client,
        Action<string> log,
        ProviderAuthLauncher? providerProbeLauncher = null)
    {
        _options = options;
        _client = client;
        _log = log;
        _providerProbeLauncher = providerProbeLauncher;
    }
        ...
        CancellationToken ct)
    {
        ...
        if (_providerProbeLauncher is null)
            return new ProviderLimitProbeResult(false, "No provider recovery probe launcher is configured.");
        ...
        try
        {
            result = await _providerProbeLauncher(binary, arguments, bounded.Token);
        }
