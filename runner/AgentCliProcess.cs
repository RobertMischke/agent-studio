...
/// <summary>
/// Resolves which coding-agent CLI invocation a remote run uses: the card's
/// execution spec decides where it can, the host's configured binaries fill the
/// gaps. Pure resolution only — since T1 (AGT-2370) nothing in this file starts
/// a process; the CAR engine builds the argv from the typed fields and the
/// legacy engine consumes <see cref="CliInvocation.Arguments"/> inside the
/// detached worker.
/// </summary>
public static class AgentCliProcess
{
    /// <summary>
