...
public sealed record ProviderCredentialFreshness(
    ...
    bool NonInteractiveRefreshAvailable = false);
...
public static class ProviderCredentialFreshnessProbe
{
    ...
    internal static ProviderCredentialFreshness? ReadClaude(string path)
    {
        ...
        return new(
            ...
            refreshExpiry is not null
                ? "Claude refresh-credential expiry is known; the CLI status probe can refresh non-interactively before that deadline."
            ...
            NonInteractiveRefreshAvailable: canRefresh);
    }
    ...
    internal static ProviderCredentialFreshness? ReadCodex(string path)
    {
        ...
        var canRefresh = root.TryGetProperty("tokens", out tokens)
                         && tokens.ValueKind == JsonValueKind.Object
    ...
    private static bool HasNonEmptyString(JsonElement parent, string property)
