...
public static class ProviderAuthFailureClassifier
{
    private static readonly Regex ExplicitSignedOut = new(
        """(?:not\s+(?:logged|signed)\s+in|logged\s+out|no\s+(?:active\s+)?session|not\s+authenticated|no\s+(?:stored\s+)?credentials|login\s+required|please\s+log\s*in|please\s+sign\s+in|re-?authenticate|invalid\s+(?:credentials|api\s*key)|(?:oauth|access|refresh)\s+token\s+(?:is\s+)?(?:expired|revoked)|token\s+has\s+expired)""",
    ...
    private static readonly Regex Transient = new(
        """(?:timed?\s*out|timeout|temporar(?:y|ily)|try\s+again|connection\s+(?:reset|refused|closed)|network\s+(?:error|unavailable)|dns|econnreset|etimedout|eai_again|socket|tls\s+handshake|service\s+unavailable|bad\s+gateway|gateway\s+timeout|\b50[234]\b|refresh\s+(?:already\s+)?in\s+progress|token\s+refresh\s+race)""",
        RegexOptions.IgnoreCase | RegexOptions.Compiled);
