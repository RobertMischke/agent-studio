...
try {
    ...
    if ($watchdogTask.Principal.LogonType -ne 'Password') {
        throw "Scheduled task '$WatchdogTaskName' must use the Password logon type."
    }
    if ($keeperTask.Principal.LogonType -ne 'Password') {
        throw "Scheduled task '$KeeperTaskName' must use the Password logon type."
    }
...
- Watchdog task logon type (must be Password): $(if ($watchdogTask) { $watchdogTask.Principal.LogonType } else { 'unavailable' })
- Keeper task logon type (must be Password): $(if ($keeperTask) { $keeperTask.Principal.LogonType } else { 'unavailable' })
