...
param(
    ...
    [ValidateRange(1024, 65535)] [int] $Port = 5072
)
...
try {
    ...
    $fields = 'projects', 'tasks', 'events', 'artifacts', 'runnerIdentities', 'codingAttempts', 'reviewAttempts', 'leases', 'authorityEpoch'
    ...
    $report = [ordered]@{
        ...
        authorityDisposition = 'Live coding and review leases import as process-unknown; positive containment is required before requeue.'
    }
    ...
    $env:LISTEN_URL = $previousEnvironment.LISTEN_URL
    $env:STORE_PATH = $previousEnvironment.STORE_PATH
    $env:BACKUP_PATH = $previousEnvironment.BACKUP_PATH
    $env:AUTH = $previousEnvironment.AUTH
}
