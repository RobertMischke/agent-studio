...
try {
    ...
    $releaseExecutable = Join-Path $releasePath 'task-server.exe'
    if (-not (Test-Path -LiteralPath $releaseExecutable -PathType Leaf)) {
        throw "Installed Task Server executable is missing: $releaseExecutable"
    }
    $releaseVersion = (& $releaseExecutable '--version').Trim()
    if ($LASTEXITCODE -ne 0 -or $releaseVersion -notmatch [Regex]::Escape($sha)) {
        throw "Installed Task Server identity does not contain commit $sha. Observed: $releaseVersion"
    }
