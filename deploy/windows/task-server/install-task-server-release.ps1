...
if ($LASTEXITCODE -ne 0 -or -not $sourceHead.Equals($ReleaseSha, [StringComparison]::OrdinalIgnoreCase)) {
...
if ($PSCmdlet.ShouldProcess($releaseDirectory, 'Publish and install the Task Server release')) {
    ...
    if ($LASTEXITCODE -ne 0 -or $publishedVersion -notmatch [regex]::Escape($ReleaseSha)) {
        throw "Published Task Server does not report the requested SHA '$ReleaseSha'."
    }
