...
public sealed class OrchestratorContextChatEndpointsTests : IDisposable
{
    ...
    public void ResolveContextPath_TaskAndDossierGetOwnFiles_ProjectAndNullShareLegacyFile()
    {
        ...
        OrchestratorContextKey.TryParse($"dossier:{Project}/context-model", out var dossier);
        ...
        Assert.Contains(Path.Combine(".orchestrator", "context-chats"), dossierPath, StringComparison.Ordinal);
        Assert.EndsWith(dossier!.Encode() + ".jsonl", dossierPath, StringComparison.Ordinal);
    }

    [Fact]
    public void Append_KeepsTaskDossierAndProjectTranscriptsIsolated()
    {
