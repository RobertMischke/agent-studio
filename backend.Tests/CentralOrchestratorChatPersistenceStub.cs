        ...
        CancellationToken ct)
    {
        lock (_gate)
        {
            ...
            if (!_turns.TryGetValue(key, out var turns))
            {
                turns = [];
                _turns[key] = turns;
                _summaries[key] = context?.TaskKey ?? context?.DossierKey ?? $"Project chat for {projectName}";
            }
        ...
        lock (_gate)
        {
            var contexts = _turns.Keys.Select(key =>
            {
                ...
                return new OrchestratorContextDto(
                    ...
                    null,
                    parsed.TaskKey,
                    _summaries.GetValueOrDefault(key) ?? parsed.TaskKey ?? parsed.DossierKey ?? $"Project chat for {project}",
                    DateTime.UtcNow,
                    DateTime.UtcNow,
                    null,
                    _turns[key].Count,
                    DossierKey: parsed.DossierKey);
            }).ToArray();
