...
public class TunnelSupervisionPolicyTests
{
    ...
    private static TunnelSupervisionSnapshot Snapshot(
        ...
        => new(
            ...
            Keeper: new TunnelKeeperStatus(
                "AgentRunner-TunnelKeeper", keeperRegistered, keeperState, keeperStatus,
                keeperObservedAt ?? Now, "ok"),
            Watchdog: new TunnelWatchdogStatus(
                "AgentRunner-TunnelWatchdog", watchdogRegistered, watchdogState,
    ...
    [InlineData("keeper")]
    [InlineData("watchdog")]
    public void RegisteredButStoppedTask_IsAttention(string component)
    {
        var snapshot = component == "keeper"
            ? Snapshot(keeperState: "Ready")
            : Snapshot(watchdogState: "Ready");
        Assert.Equal(TunnelSupervisionStatuses.Attention, TunnelSupervisionPolicy.Classify(snapshot, Now));
    }
