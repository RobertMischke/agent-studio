...
public sealed class TestRunServiceTests : IDisposable
{
    ...
    public void CardEvidence_BindsExactRemoteReviewBuildTestsGrade_WhenProjectRunIsAbsent()
    {
        ...
        Assert.Equal("All build-tests rows passed.", source.Reason);
        Assert.Equal("remote-review-grade-review-42.md", source.ReportRef);
    }

    [Fact]
    public void CardEvidence_KeepsAgT2689BuildProofGreenAndProjectsBlockingAspectSeparately()
    {
        var stack = BuildStack();
        var commit = RevParse(stack.Repo, "HEAD");
        ...
        File.WriteAllText(
            ...
            $"""
             ...
             | build-tests | pass | BaselineCompared | Review command 'verify-2' passed. |
             | requirement-fit | pass | RemoteAspectVerdict | Requirement fit passed. |
    ...
    public void CardEvidence_UsesNotProvenOnlyWhenRemoteReviewBuildTestsRowIsMissing()
    {
        var stack = BuildStack();
        var commit = RevParse(stack.Repo, "HEAD");
        var folder = Path.Combine(stack.Storage, TaskStates.HumanReview, "review-missing-build-tests");
        Directory.CreateDirectory(folder);
        File.WriteAllText(
            ...
            $"""
             ...
             | Aspect | Status | Classification | Summary |
             | --- | --- | --- | --- |
             | documentation-impact | block | RemoteAspectVerdict | Documentation was not updated. |
             """);
        var job = Task("review-missing-build-tests", stack.Storage, commit) with
        {
            State = TaskStates.HumanReview,
            FolderPath = folder,
        };
        ...
        Assert.Equal(
            "build-tests missing: No build-tests command row exists in the review report.",
            buildTests.Reason);
        Assert.False(string.IsNullOrWhiteSpace(buildTests.ReportRef));
    }
