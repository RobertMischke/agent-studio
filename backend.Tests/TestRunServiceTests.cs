...
public sealed class TestRunServiceTests : IDisposable
{
    ...
    public void CardEvidence_BindsExactRemoteReviewBuildTestsGrade_WhenProjectRunIsAbsent()
    {
        ...
        Assert.Equal("All recorded build-test commands passed.", source.Reason);
        Assert.Equal("remote-review-grade-review-42.md", source.ReportRef);
    }

    [Fact]
    public void CardEvidence_SeparatesPassingBuildTestsFromBlockingReviewAspect()
    {
        var stack = BuildStack();
        var commit = RevParse(stack.Repo, "HEAD");
        ...
        File.WriteAllText(
            ...
            $"""
             ...
             | requirement-fit | pass | Verified | The implementation fits the requirement. |
             | code-quality | pass | Verified | The code quality is acceptable. |
             | tests-and-evidence | pass | Verified | Test evidence is sufficient. |
             | documentation-impact | block | ProductFinding | Public API and state-file contract changed without corresponding load-bearing doc updates. |

             ## Command evidence

             | Phase | Workspace | Step | Shell | Exit | Consumed budget |
             | --- | --- | --- | --- | ---: | ---: |
             | verification | candidate | verify-1 | `bash -lc` | 0 | 1,000 ms |
             | verification | candidate | verify-2 | `bash -lc` | 0 | 528,000 ms |
             """);
    ...
    [Fact]
    public void CardEvidence_ExplainsWhichVerificationCommandHasNoBuildTestsVerdict()
    {
        var stack = BuildStack();
        var commit = RevParse(stack.Repo, "HEAD");
        var folder = Path.Combine(stack.Storage, TaskStates.HumanReview, "missing-review-proof");
        Directory.CreateDirectory(folder);
        File.WriteAllText(
            ...
            $"""
             ...
             | Aspect | Status | Classification | Summary |
             | --- | --- | --- | --- |
             | documentation-impact | block | ProductFinding | Required documentation is missing. |

             | Phase | Workspace | Step | Shell | Exit | Consumed budget |
             | --- | --- | --- | --- | ---: | ---: |
             | verification | candidate | verify-2 | `bash -lc` | 0 | 10 ms |
             """);
        var job = Task("missing-review-proof", stack.Storage, commit) with
        {
            State = TaskStates.HumanReview,
            FolderPath = folder,
        };
        ...
        Assert.Equal("Build-tests verdict missing for verify-2.", buildTests.Reason);
        Assert.Equal("remote-review-grade-review-missing.md", buildTests.ReportRef);
    }
