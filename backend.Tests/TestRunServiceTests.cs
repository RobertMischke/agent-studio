...
public sealed class TestRunServiceTests : IDisposable
{
    ...
    public void CardEvidence_BindsExactRemoteReviewBuildTestsGrade_WhenProjectRunIsAbsent()
    {
        ...
        Assert.Equal("All reported build-tests rows passed.", source.Reason);
    ...
    public void CardEvidence_SeparatesPassingBuildTestsFromBlockingReviewAspect()
    {
        var stack = BuildStack();
        var commit = RevParse(stack.Repo, "HEAD");
        var folder = Path.Combine(stack.Storage, TaskStates.Archive, "agt-2689-review-evidence");
        Directory.CreateDirectory(folder);
        const string attemptId = "review_ad5cca8e3178425fb9ba9cabe329d50e";
        var reportName = $"remote-review-grade-{attemptId}.md";
        File.WriteAllText(
            ...
            $"""
             ...
             | requirement-fit | pass | RemoteAspectVerdict | Requirements fit. |
             | code-quality | pass | RemoteAspectVerdict | Code quality passed. |
             | tests-and-evidence | pass | RemoteAspectVerdict | Test evidence passed. |
             ...
             | --- | --- | --- | --- | --- | --- | ---: | --- | --- | --- |
             | verification | candidate | verify-1 | remote | host / executor | `dotnet build` | 0 | review-command | stdout | stderr |
             | verification | candidate | verify-2 | remote | host / executor | `dotnet test` | 0 | review-command | stdout | stderr |
             """);
    ...
    [Fact]
    public void CardEvidence_UsesNotProvenOnlyWhenBuildTestsRowIsMissing()
    {
        var stack = BuildStack();
        var commit = RevParse(stack.Repo, "HEAD");
        var folder = Path.Combine(stack.Storage, TaskStates.Archive, "missing-build-tests-row");
        Directory.CreateDirectory(folder);
        File.WriteAllText(
            Path.Combine(folder, "remote-review-grade-review-missing-build.md"),
            $"""
             ---
             type: remote-review-grade
             attemptId: "review-missing-build"
             ...
             | Aspect | Status | Classification | Summary |
             | --- | --- | --- | --- |
             | documentation-impact | block | RemoteAspectVerdict | Pipeline documentation is missing. |
             ...
             | verification | candidate | verify-1 | remote | host / executor | `dotnet build` | 0 | review-command | stdout | stderr |
             """);
        var job = Task("missing-build-tests-row", stack.Storage, commit) with
        {
            State = TaskStates.Archive,
            FolderPath = folder,
        };
        ...
        var buildTests = evidence.Sources.First(source => source.Kind == "review-build-tests");
