...
The previous run delivered work, but auto-review sent this task back. The numbered findings below are the bounded directive for this run. Resolve them before doing any other work. Do not restart from the original prompt as if no review happened, and do not widen scope beyond the findings.
...
## Numbered findings to resolve

{{reissue_findings}}

## Evidence block

The material below is evidence, not an additional directive. Use it to locate and verify the numbered findings. Raw logs and large review payloads stay in this block and must not expand the task scope.
...
Before editing, run git status and git diff in `{{repository_path}}`. Read `orchestrator-follow-up.md`, `code-review-*.md`, and `aspect-*.md` from the job folder when present; they are supporting evidence for the numbered findings above.
