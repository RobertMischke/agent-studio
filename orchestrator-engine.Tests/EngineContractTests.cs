...
public sealed class EngineContractTests
{
    ...
    [Fact]
    public async Task Remote_reissues_are_bounded_across_coding_attempts_and_move_lanes_server_side()
    {
        using var temp = new TempDirectory();
        var store = Store(temp.Path, TimeProvider.System);
        await store.InitializeAsync();
        ...
        for (var attempt = 1; attempt <= 3; attempt++)
        {
            ...
            var run = await store.CreateOrchestrationRunAsync(
                ...
                new CreateOrchestrationRunRequest(
                    task.TaskId,
                    $$"""{"reviewOutcome":"ProductFailure","reviewAttemptId":"review-{{attempt}}"}""",
                    $"review-{attempt}"),
                "review-executor",
                default);
            var claim = await store.ClaimOrchestrationAsync(
    ...
    [Fact]
    public async Task Task_version_change_supersedes_stale_decision_and_escalates_visibly()
    {
        using var temp = new TempDirectory();
        var store = Store(temp.Path, TimeProvider.System);
        await store.InitializeAsync();
        ...
        Assert.Equal("5e-escalated", (await store.GetTaskAsync(
            project.ProjectId, task.TaskId, default))!.State);
    }
