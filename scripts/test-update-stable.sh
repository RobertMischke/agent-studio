...
task_server_acceptance_marker=$test_root/task-server-acceptance-ran
...
EOF
cat > "$devspace/verify-task-server-cutover.sh" <<'EOF'
#!/usr/bin/env sh
set -eu
test -d "$1"
test -n "$2"
test "$3" = 'http://127.0.0.1:5071'
test "$4" = 'http://127.0.0.1:5031'
if [ "${ATP_TEST_ACCEPTANCE_FAIL:-}" = 1 ]; then
  exit 42
fi
: > "$ATP_TEST_TASK_SERVER_ACCEPTANCE_MARKER"
EOF
...
chmod +x "$devspace/stop-stable.sh" "$devspace/start-stable.sh" \
  "$devspace/start-task-server.sh" "$devspace/deploy-task-server.sh" \
  "$devspace/verify-task-server-cutover.sh" "$fake_bin/npm"
...
run_update() {
  env \
    ...
    ATP_TEST_TASK_SERVER_ACCEPTANCE_MARKER="$task_server_acceptance_marker" \
    ATP_TEST_ACCEPTANCE_FAIL="${ATP_TEST_ACCEPTANCE_FAIL:-}" \
...
test -f "$task_server_acceptance_marker"
...
# path must remain detached when the functional acceptance gate fails.
rm -f -- "$task_server_start_marker"
rm -f -- "$task_server_acceptance_marker"
set +e
acceptance_output=$(ATP_TEST_ACCEPTANCE_FAIL=1 run_update)
acceptance_rc=$?
set -e
test "$acceptance_rc" -ne 0
test ! -f "$task_server_acceptance_marker"
if git -C "$stable_checkout" symbolic-ref --quiet --short HEAD >/dev/null; then
  printf '%s\n' 'failed acceptance unexpectedly unpinned Stable' >&2
  exit 1
fi

# Once the complete gate passes, the recovery path may attach main.
noop_output=$(run_update)
test -f "$task_server_start_marker"
test -f "$task_server_acceptance_marker"
printf '%s' "$noop_output" | grep -q 'rerunning the complete cutover verification'
test "$(git -C "$stable_checkout" symbolic-ref --short HEAD)" = main
