...
requires host-owned `deploy-task-server.sh`, `start-task-server.sh`, and
`verify-task-server-cutover.sh` wrappers.
...
probes and the host's claim/review/report/board acceptance gate all pass; a
failed candidate is not attached to `main`.
...
`Start-ScheduledTask -TaskName AgentOrchestrator-TaskServer`. The acceptance
wrapper performs the fenced functional round trip and persists its evidence.
All three wrappers run outside interactive sessions and propagate failures.
