...
grep -Fq -- "-Password \$password" "$registration"
grep -Fq "Get-Credential" "$registration"
...
grep -Fq "consecutiveHealFailures" "$watchdog"
...
grep -Fq "operatorAlarmRaised" "$watchdog"
grep -Fq -- "-Password \$password" "$watchdog_registration"
grep -Fq "Get-Credential" "$watchdog_registration"
grep -Fq -- "-AtStartup" "$watchdog_registration"
grep -Fq -- "-ExecutionTimeLimit ([TimeSpan]::Zero)" "$watchdog_registration"
grep -Fq "Register the keeper before the watchdog" "$watchdog_registration"
grep -Fq "python3 -m http.server" "$forced_kill"
grep -Fq "RecoveryDeadlineSeconds = 150" "$forced_kill"
grep -Fq "forced-kill-test.md" "$forced_kill"
grep -Fq "event=probe status=failed" "$forced_kill"
grep -Fq "event=heal-result status=healthy" "$forced_kill"
grep -Fq "Reset-WatchdogCounters" "$forced_kill"
grep -Fq "EvidenceDirectory = \$env:JOB_RESULTS_DIR" "$forced_kill"
grep -Fq "must use the Password logon type" "$forced_kill"
...
printf 'Tunnel keeper and watchdog assets contain functional probes, two-strike healing, cgroup-aware remote listener cleanup, exit capture, alarms, and password-backed session-independent task registration.\n'
