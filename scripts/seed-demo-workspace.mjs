#!/usr/bin/env node
// Idempotent seed for the slim DEV demo datastore (ADR-0053).
//
...
function writeTask(root, task, index) {
  ...
  writeText(
    join(dir, 'prompt.md'),
    `# ${task.title}\n\nDemo task seeded by scripts/seed-demo-workspace.mjs for the slim DEV demo store (ADR-0053). Not real work — safe to edit, move, or delete; a re-seed restores it.\n`
  );
