...
function options(argv) {
  ...
  if (result.expectedSha && !/^[0-9a-f]{40}$/.test(result.expectedSha)) {
    throw new Error('--expected-sha must be a 40-character Git SHA.');
  }
  return result;
}
...
async function verifyVersion(response, expectedSha, label) {
  ...
  const version = typeof status.serverVersion === 'string' ? status.serverVersion.toLowerCase() : '';
  if (!version.includes(expectedSha)) {
    throw new Error(`${label} reports '${status.serverVersion ?? 'no version'}', expected SHA ${expectedSha}.`);
  }
}
...
async function run() {
  ...
  const directStatus = await probe(
    `${value.taskServerUrl}/api/v1/management/status`,
    headers,
    deadline,
    'Task Server management plane',
  );
  await verifyVersion(directStatus, value.expectedSha, 'Task Server management plane');
  if (!value.directOnly) {
    const proxyStatus = await probe(
      `${value.backendUrl}/api/v1/management/status`,
      {},
      deadline,
      'OrchestratorApi Task Server proxy',
    );
