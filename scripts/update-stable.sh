...
task_server_status() {
  ...
  curl -fsS --max-time 10 \
    ...
    -H 'X-Client-Id: stable-updater' \
    -H 'X-Actor-Id: stable-updater' \
    "$task_server_url/api/v1/management/status"
}

task_server_matches_target() {
  status=$(task_server_status 2>/dev/null) || return 1
  printf '%s' "$status" | grep -Fq "$target"
}
...
prepare_task_server_shutdown() {
  ...
  log "Creating Task Server backup before package replacement"
  backup=$(task_server_request POST /api/v1/management/backups \
    '{"name":"before-stable-release-update"}')
  printf '%s' "$backup" | grep -qE '"sha256"[[:space:]]*:[[:space:]]*"[0-9a-fA-F]{64}"' \
    || fail "Task Server did not return a verified pre-update backup digest; Stable was not stopped."
}
...
task_server_upgrade=0
task_server_available=0
if [ "$task_server_required" -eq 1 ]; then
  if curl -fsS --max-time 10 "$task_server_url/readyz" >/dev/null 2>&1; then
    task_server_available=1
  fi
  if [ "$task_server_available" -eq 1 ] && task_server_matches_target; then
    log "Supervised Task Server already matches candidate $target; keeping it running"
  else
    task_server_upgrade=1
    if [ "$task_server_available" -eq 1 ]; then
      prepare_task_server_shutdown
    else
      log "Task Server is not ready; package installation will recover or create the supervised service"
    fi
  fi
fi
log "Stopping Stable"
...
if [ "$task_server_upgrade" -eq 1 ]; then
...
if [ "$task_server_upgrade" -eq 1 ]; then
  ...
  run_task_server_asset "$task_server_control_script" \
    -Action Start \
    -ReadyUrl "$task_server_url/readyz"
elif [ "$task_server_required" -eq 1 ]; then
  log "Waiting for preserved Task Server before Stable API startup"
  wait_for_http "Task Server readiness" "$task_server_url/readyz"
fi
