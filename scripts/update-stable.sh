...
#   ATP_TASK_SERVER_ACCEPTANCE_SCRIPT host-owned claim/review/report/board gate
...
task_server_acceptance_script=${ATP_TASK_SERVER_ACCEPTANCE_SCRIPT:-$devspace_dir/verify-task-server-cutover.sh}
...
[ -x "$task_server_acceptance_script" ] || fail "Task Server acceptance script is not executable: $task_server_acceptance_script"
...
if [ "$head_before" = "$target" ]; then
  log "Stable already matches $stable_remote/$stable_branch; rerunning the complete cutover verification."
  ...
  "$task_server_acceptance_script" "$stable_checkout" "$target" "$task_server_url" "$backend_url"
...
log "Running claim, review, report, and board acceptance gate"
"$task_server_acceptance_script" "$stable_checkout" "$target" "$task_server_url" "$backend_url"
