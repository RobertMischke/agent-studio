...
Delivered (Multichat Phase 3): the optional chat switcher is collapsed by default behind an active-or-queued count chip. Frame B expands it beside the uninterrupted chat with searchable Global / Projects / Tasks groups, runtime, parked, local-unread, and cumulative-token badges. Row names switch chat context without moving the workspace; the separate arrow navigates to the global board, project board, or task tab. The oversized side-sheet controller is split so context/rail hosting and transcript/composer behavior have separate component boundaries. See [docs/concepts/orchestrator-chat.md](./docs/concepts/orchestrator-chat.md#chat-switcher-rail).
