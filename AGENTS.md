...
**Anti-endless-reissue circuit breaker (ADR-0002 safety net).** A run that fails without progress could otherwise be re-issued forever: a soft, non-routed failure (`classifier-unknown`, `missing-terminal-sentinel`, `no-agent-output`) is re-issued as a `UserContinue` run or left in `3-progress` for the next auto-pickup tick, and neither path feeds the auto-pickup failure breaker (the re-issue returns before the counter and is not an `AutoPickup` intent). Two guards stop that loop, both routing to `5-human-review` through the `HumanReviewEscalation` funnel (never a raw `MoveJob`):

- **Context-overflow is non-retryable, quarantined on first detection.** `AgentOutcomeAnalyzer` types a *failed* run whose CLI output carries a prompt-too-long / context-length / request-too-large / `413` needle (`IsContextOverflow`) as `RunIssueKind.ContextOverflow`, *before* the generic classifier-unknown path and gated on `failed` (so a mid-success quote is unaffected). `RunOutcomePolicy` routes it straight to `NotifyUserAndStop` - no soft intervention, no retry budget - because re-issuing resends the same or larger context and overflows identically. ("Prompt too long" reaches the buffer because `ClaudeOutputRenderer` renders the result frame as a stderr line, so no CLI-adapter change is needed.)
- **Per-task no-progress quarantine after N consecutive failures.** The pure [`RunQuarantineBreaker`](backend/Services/Runner/RunQuarantineBreaker.cs) (`DefaultFailThreshold = 3`) counts no-progress failures per task across BOTH the auto-pickup run and the `UserContinue` re-issue it spawns. `ProjectRunner.OnCliFinished` increments an in-memory `_consecutiveFailNoProgress` counter for a run that did not reach review, was not deliberately stopped, produced **zero commits**, and carries a countable issue kind (`RunQuarantineBreaker.CountsAsNoProgressFailure` - an exclusion list so a future soft-failure kind counts by default; kinds that route on their own, have their own breaker, or are accepted as done are excluded). On reaching the threshold it overrides the action to a terminal `RunIssueKind.Quarantined` stop. **Reset on progress:** a new commit or reaching review clears the counter, so a healthy long-running task that keeps committing is never quarantined; the counter is also cleared when the card leaves the run loop for human review.

Both `ContextOverflow` and `Quarantined` surface on the board as outcome-issue chips (via `TaskScannerService.TryResolveOutcomeIssue` needles), carry their own `HumanReviewEscalationCategories`, are excluded from the abort-review rerun gate, and emit a `[circuit-breaker] quarantined <job> after N fails (reason=...)` log line. When you add a new soft-failure `RunIssueKind`, decide explicitly whether it should count toward the quarantine streak (it does by default) and keep `RunQuarantineBreakerTests` green.
