...
/// <summary>Pure wire-level policy shared by both capability stores.</summary>
public static class ProviderAuthCapabilityPolicy
{
    /// <summary>
    /// A routine ready advertisement heals an old provider-auth failure only
    /// when the runner says a live probe actually authenticated. Last-good
    /// readiness retained through a transient error is intentionally excluded.
    /// </summary>
    public static bool IsRecoveryProof(
        string capabilityKey,
        string advertisedStatus,
        string? operationalState)
        => capabilityKey.StartsWith("provider-auth:", StringComparison.Ordinal)
           && string.Equals(advertisedStatus, "ready", StringComparison.Ordinal)
           && operationalState is ProviderAuthOperationalStates.Authenticated
               or ProviderAuthOperationalStates.CredentialsExpiring;
}
