...
- [agent-task-contract.md](../../docs/system/contracts/agent-task.md) - the application and CLI-agent ownership boundary
...
Create or update an `AGENTS.md` in that project root. Copy the content from `docs/system/contracts/agent-task.md` into an `## Agent Task Contract` section. `AGENTS.md` is the **single source of truth** and is read natively by Codex CLI, Claude Code, and the GitHub Copilot coding agent.
...
- Use the **exact same content** from `docs/system/contracts/agent-task.md`; do not paraphrase or modify it.
