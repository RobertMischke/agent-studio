...
1. **One source, two views.** The deep explanation lives in a Markdown doc under `docs/` (e.g. `docs/system/cli/skills/sandbox-and-yolo.md`). The UI embeds the short version inline (or even renders the relevant Markdown section in-place). When the doc changes, the UI updates without copy-paste drift.
...
The next-generation chat mockup family (`mockups/chat-window-next-gen`, retired 2026-07 as design history) informed this direction.
...
The runs file layout is documented in [filesystem-contract.md](../system/contracts/filesystem.md). When the runtime changes, the contract moves with it.
...
The project Kanban board has a single visual specification. Every layout change to the board must reconcile with it before touching CSS, components, or grid math. The spec, its locked-decision taxonomy, interactive reference, and reconciliation table lived in the `mockups/kanban-board-design/` family (retired 2026-07 as design history). The first-slice CSS lands behind `Frontend:KanbanDesignSpecV1`, default off.
