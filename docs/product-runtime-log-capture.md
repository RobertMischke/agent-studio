...
(`docs/product-runtime-observability.md`, queued in
...
| `<job>/results/runtime/*.jsonl` | Harvested into the job folder when `JOB_RESULTS_DIR` is set | Test-derived evidence behaves like screenshots. See [docs/protocol-style.md §4](protocol-style.md#image-flow). |
...
1. **Job folders live in the watched workspace**, not in this repository
   (`docs/filesystem-contract.md`). Anything under
...
2. **The watched workspace's `.gitignore` already excludes
   `**/results/`** (see `docs/protocol-style.md §4.3`), and the same
