...
*Fallback-Semantik:* Jedes Feld ist optional. Ein Server ohne T0b sendet kein `runSpec`, ein Runner ohne T0b ignoriert es — in beiden Fällen entscheiden `RUNNER_CLI_BIN` / `RUNNER_CLI_ARGS` wie bisher. Der Runner setzt die Spec in `AgentCliProcess.Resolve` in Argumente um (claude: `--model X` + `--effort L`; codex: `-m X` + `-c model_reasoning_effort="L"` + `-` für den Prompt auf stdin) und nimmt `RUNNER_CLI_ARGS` nur noch als Basis, solange die Karte auf demselben CLI bleibt; für ein CLI, für das der Host kein Binary hat, gewinnt die Konfiguration sichtbar (`note=…`). Die Spec wandert in den Slot (`PersistedRunnerSlot.RunSpec`) und in die `spec.json` des Detached Workers, beide additiv — eine vor T0b geschriebene Datei lädt mit `null` und läuft unverändert weiter, was den Hot-Deploy aus §4 zulässt. Beweiszeile beim Spawn (Journal **und** Task-Log): `[runner] spec cli=… model=… thinking=… permission=… context=… source=card|runner-options`.
