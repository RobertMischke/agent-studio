...
assigned ports without changing the product path under test.
...
JSON. The durable command and machine-readable output format are kept in the
smoke script so the claim can be repeated for every release candidate.
