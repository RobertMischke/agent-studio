...
| Agent Host | one `agent-host` daemon per host | claim → worktree → CLI slot → deliver (result-SHA handoff), salvage | decide review outcomes |
...
`Agent Host` names the per-host daemon. The persisted `runner-id` remains its
execution identity and is not renamed during this migration.
...
- **Agent Host**: start: register, poll. crash: leases expire, server offers work again; salvage branches keep partial work. deploy: per host, independent.
