...
- **Signed out**: consecutive distinguishable auth failures confirmed that the
  CLI is genuinely signed out. Hover for the exact re-auth detail.
- **Unavailable**: the provider capability is unavailable for a reason other
  than a confirmed sign-out, such as a missing binary.
...
`Signed out` state creates the blocking re-auth notification. Rate-limit output
