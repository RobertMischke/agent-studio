...
5. Install a third host-owned `verify-task-server-cutover.sh` wrapper and set
   `ATP_TASK_SERVER_ACCEPTANCE_SCRIPT` when its path differs from the devspace
   default. The updater calls it with `<stable-checkout> <target-sha>
   <task-server-url> <backend-url>`. It must exit successfully only after it has
   exercised one fenced coding claim through completion, one immutable review
   ...
   the task's collected `results/` directory. A missing or failed acceptance
   wrapper leaves Stable detached at the candidate SHA.
