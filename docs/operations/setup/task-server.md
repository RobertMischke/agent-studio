...
The cutover probe compares the Task Server version reported directly and
through OrchestratorApi with the exact candidate Git SHA. Retrying a held,
detached candidate reinstalls that package before probing, so an older healthy
service can never satisfy the unpin gate.
