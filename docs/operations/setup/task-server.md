...
1. Copy the complete legacy `TaskRepository`, including `identities/`,
   ...
   On Windows, run the packaged rehearsal against the installed executable and
   write its JSON report directly to the task's collected results directory:
   ...
   .\deploy\windows\task-server\test-legacy-migration-dry-run.ps1 `
       ...
       -ReportPath C:\path\to\task-results\task-server-migration-dry-run.json
   ```

   The script copies the source before starting the disposable service, so the
   rehearsal never writes to the live legacy root.
...
4. Install two host-owned shell wrappers beside the dev and Stable checkouts.
   ...
   and browser boot. A detached held checkout advances to the target while
   remaining detached and is attached to `main` only after every topology and
   browser probe succeeds. A failed rollout remains pinned for recovery.
