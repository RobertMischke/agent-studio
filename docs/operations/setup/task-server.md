...
`AgentOrchestrator-TaskServer` with `LogonType S4U`. Mutable authority lives outside
...
`scripts/update-stable.sh` owns later package updates. On Windows it preserves
an already-current supervised Task Server. When the candidate has a different
Task Server package, it drains authority, verifies a pre-update backup, stops
Stable and Task Server, installs the candidate package, and starts Task Server
first. In either path it waits for `/readyz` before starting the Stable API.
Its extended boot proof checks direct readiness, API health, the `/api/v1`
proxy, the board projection, the management proxy, and the browser. After every
check passes it returns Task Server to `Normal` and attaches Stable to local
`main` tracking `origin/main`. A failed candidate therefore cannot silently
remove a rollback pin.
...
2. Rehearse a full import against a copied legacy root and an isolated Task
   ...
   & "$assets\rehearse-legacy-migration.ps1" `
     ...
     -EvidenceDirectory 'C:\Projects\agent-taskboard-cutovers\AGT-2663' `
...
3. Stop every legacy writer, including Stable API pickup and every Runner that
   ...
   & "$assets\invoke-legacy-migration.ps1" `
     ...
     -Import `
     -FreezeConfirmed `
     -EvidencePath 'C:\Projects\agent-taskboard-cutovers\AGT-2663\production-import.json'
   ```
...
5. Run the versioned `scripts/update-stable.sh`. It deploys current `main`,
   preserves an already-current supervised Task Server or upgrades it when
   needed, waits for Task Server before starting the API, performs the extended
   boot proof, changes Task Server to `Normal`, and reattaches the verified
   Stable checkout to `main`.
6. Let the real Coding and Review executors process post-cutover work. Then
   ...
   & "$assets\verify-task-server-cutover.ps1" `
     ...
     -EvidencePath 'C:\Projects\agent-taskboard-cutovers\AGT-2663\end-to-end.json'
   ```
