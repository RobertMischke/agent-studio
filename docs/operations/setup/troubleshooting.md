...
**Manual break-and-heal rehearsal** (for verifying this on the actual Windows control-plane host - the automated tests (`backend.Tests/NpmShimHealerFallbackPolicyTests.cs`, `NpmShimHealerNpmInstallFallbackTests.cs`, `NpmShimHealerVersionCaptureTests.cs`, `CliRepairGateTests.cs`, `CliRepairCooldownPolicyTests.cs`, `WorkspaceSummaryCliRepairTests.cs`) only exercise the OS-independent seams, since `NpmShimHealer` short-circuits on non-Windows hosts):
