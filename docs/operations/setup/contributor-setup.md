...
| `claude` / `gemini` command is missing or broken on Windows after an interrupted npm update | Half-completed npm install left orphan shim files or a stub binary, or a racing auto-update deleted the global bin launcher outright | `bash tools/check-cli-shims.sh` - self-heals and re-verifies with `claude --version`. When the launcher is gone outright with the package still present it runs a bounded `npm install -g` reinstall (max once/hour); check `%APPDATA%\npm\.atp-npm-reinstall-journal.jsonl` for the attempt history if it keeps recurring. |
