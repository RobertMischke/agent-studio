...
| `claude` / `codex` command shim is missing on Windows while its npm package remains installed | An interrupted install or CLI update removed the global `.cmd` launcher | The local capability probe runs one `npm install -g` repair attempt per hour and records `logs/backend/cli-repairs.jsonl`. Only a failed repair alarms. |
| `claude` / `gemini` command is broken by an orphan shim or stub binary | A half-completed npm install left atomic-rename debris or a stub executable | `bash tools/check-cli-shims.sh` repairs the older orphan and stub shapes and re-verifies with `claude --version`. |
