...
| `claude` / `gemini` command is missing or broken on Windows after an interrupted npm update | Half-completed npm install left orphan shim files or a stub binary | `bash tools/check-cli-shims.sh` - self-heals and re-verifies with `claude --version`. The running local backend also reinstalls Claude or Codex once per hour when the package is present but its executable shim vanished; inspect `<TaskRepository>/logs/local-cli-repairs.jsonl`. |
