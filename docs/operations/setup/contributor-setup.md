...
| `claude` / `codex` disappeared from `PATH` while its npm package is still installed | The npm bin shims were removed without leaving rename orphans; the CLI's own auto-update is the suspected trigger | The backend repairs this by itself within five minutes and notes it in the status bar. To force it: `POST /api/v1/host-health/cli/claude/repair`. Background: [local-cli-self-heal.md](./local-cli-self-heal.md). |
