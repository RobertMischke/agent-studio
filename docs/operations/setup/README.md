...
| [windows-control-plane-host.md](./windows-control-plane-host.md) | The Windows sibling to linux-runner-host.md: guided registration and live status for the tunnel keeper and watchdog Scheduled Tasks that keep a Windows-hosted Studio's reverse SSH route to a Linux agent host alive, including the one-time elevation consent step. |
