...
- **Claude CLI repair on Windows after an interrupted update.** The local capability probe distinguishes a truly absent package from an intact `%APPDATA%\npm\node_modules\@anthropic-ai\claude-code` package with missing `claude`, `claude.cmd`, and `claude.ps1` launchers. The latter is repaired automatically with `npm install -g @anthropic-ai/claude-code`, at most once per hour. The same contract covers Codex with `@openai/codex`. Every attempt is recorded in `<TaskRepository>/.runtime/cli-repairs.jsonl` with before/after versions, launcher and package timestamps, nearby npm debug-log evidence, updater artifacts, relevant processes, and bounded command output. A successful repair appears quietly as `CLI repaired at <time>`; only a failed repair raises an Activity alarm.

For an operator-controlled Windows rehearsal, run `CONFIRM_CLI_SHIM_REHEARSAL=claude tools/rehearse-local-cli-repair.sh claude http://127.0.0.1:5030` from Git Bash. The script moves only the selected launchers to a temporary recoverable directory, polls the runner status for up to three minutes, verifies the restored command and version, and restores the original launchers on failure. Repeat with `codex` after the one-hour per-CLI repair window.
