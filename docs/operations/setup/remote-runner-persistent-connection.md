...
directories. Calling the two scripts directly still works and takes the
parameters the wrapper forwards:
...
When registration comes from the guided wrapper, the watchdog also refreshes
Studio's combined supervision snapshot after every probe through the
unprivileged `-StatusOnly` path. The Execution Hosts view therefore tracks
task registration/state and the latest probe/heal without a manual refresh
script becoming part of normal operation.
