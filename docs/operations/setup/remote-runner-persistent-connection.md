...
the keeper and the separate `AgentRunner-TunnelWatchdog` use Task Scheduler's
password logon, so they survive sign-out and retain the network access required
by SSH. S4U is deliberately not used because
[Windows denies network access to S4U tasks](https://learn.microsoft.com/en-us/windows/win32/taskschd/principal-logontype).
The registration prompts once when `-Credential` is omitted; the password is
passed to Task Scheduler and is not included in the task action or logs. The
tasks use the current Windows identity because that identity owns the SSH key.
Confirm that the key and SSH configuration are readable without an interactive
profile prompt. A machine that needs a different security boundary should use a
dedicated service identity with its own protected SSH key.
...
4. Preserve the generated `forced-kill-test.md`. It records elapsed recovery,
   both password-logon principals, the watchdog journal tail, and the keeper SSH
   exit tail.
