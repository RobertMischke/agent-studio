...
- **Execution Hosts shows `Task Server route unreachable`** - the
  ...
  For the Windows watchdog path, also inspect the devspace
  `.tunnel-watchdog.log`. The keeper's timestamped SSH output and exit code are
  in `%LOCALAPPDATA%\AgentTaskboard\tunnel-keeper\ssh-exit.log`.
