...
| [telemetry-layer/](telemetry-layer/index.html) | Decision dossier for a local-first telemetry layer across organization apps: current signals, content-free event contract, hybrid file buffer plus local push, Watcher action rules, Agent Studio ops-hub boundaries, and a Quality Studio pilot (AGT-2661). |
