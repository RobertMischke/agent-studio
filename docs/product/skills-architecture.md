...
The `runtime-log-analysis` skill is the canonical example of a read-only analysis skill that pairs with a runtime evidence stream. Its per-report contract specialises [`docs/reports/analysis-reports.md`](../reports/analysis-reports.md) for one topic (`runtime-observability`); follow that pattern when adding analysis skills against new evidence streams.
