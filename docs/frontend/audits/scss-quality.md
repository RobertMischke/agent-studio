...
6. `<app-update-banner>` moved out of `app.html` and is now mounted inside `<app-notification-stack>` so the update strip docks alongside the toast stack instead of being an app-shell-level concern. Each mode (`done` / `done-no-change` → `kind="success"`, `failed` → `kind="error"`) renders through `<app-notification layout="banner">`. Picker + verification-failure list project into the default slot; reload / dismiss / roll-back / picker buttons live in the `[notification-actions]` slot. Local SCSS drops the per-mode rgba/hex literals and only owns banner-host positioning + token-driven picker/button chrome.
7. `.failed-pickup-banner` is gone from `app.scss`. The caller now renders `<app-notification kind="warning" layout="banner" role="button" tabindex="0">` directly, with keyboard handlers (`enter` / `space`) and the click handler bound to the notification host. A small generic `.board-banner` host class only positions the banner above the dashboard (margin + width). The legacy `.failed-pickup-banner__dot` is removed (the notification primitive owns the icon bubble); the `failed-pickup-pulse` keyframes still highlight the lane on scroll but now derive their shadow colour from `var(--notify-warning-icon-fg)` via `color-mix` instead of a hardcoded amber rgba. (Superseded by ADR-0051: the failed-pickup banner and the `failed-pickup-pulse` keyframes were later removed entirely along with the lane.)
8. `.triage-toast` is gone from `app.scss` AND from `app.html`. The panel-anchored pill was redundant with the notification-stack toast that `JobSelectionService.showTriageToast` already raises through `NotificationService` — keeping both meant the user saw a duplicate toast on every triage action. Removing the pill leaves a single source of truth in the unified notification stack.
9. `frontend/e2e/system/f40-update-banner-themes.spec.ts` locks the dark + light contract for all three migrated surfaces: each renders the right `.notification--<kind>` + `.notification--layout-<layout>` BEM modifiers, body text clears WCAG AA (4.5:1) against the effective banner surface, severity border carries non-trivial alpha, and update-banner action buttons (`Reload`, `Dismiss`, `Roll back`, `Other runs…`) clear WCAG AA in both themes. The contrast math lives in `frontend/e2e/helpers/contrast.ts` so future banner tests can re-use it.
...
- [design-principles.md](../../product/design-principles.md) — UX rules that the SCSS supports.
