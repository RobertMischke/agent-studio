...
These files are the current proto-skill set. The broader portable skill architecture is documented in [../skills-architecture.md](../../../concepts/skills-architecture.md): long term, standard and project-specific skills should live in the task processor's central skill library, with watched projects exposing a README lookup section so direct CLI sessions can discover them too. Until that migration exists, `docs/system/cli/skills/` remains the active source for CLI-driver operational skills.
...
1. **Scaffolding lock** — [`backend.Tests/CliSkillFilesTests.cs`](../../../../backend.Tests/CliSkillFilesTests.cs). Walks `docs/system/cli/skills/`, asserts every expected file exists, every file has the frontmatter and a unique sentinel, and the body echoes the sentinel back. Free; runs on every backend test pass.
...
6. Update [`docs/system/cli/supported-clis.md`](../supported-clis.md)'s skill-pointer paragraph.
