...
2. **Windows npm shim quirk.** The npm-installed binary is `node_modules/@anthropic-ai/claude-code/bin/claude.exe`. An interrupted update can leave the package present while `claude.cmd` is absent, or leave a `claude.exe.old.<timestamp>` and no canonical binary. The local capability monitor now repairs only the package-present/missing-command-shim shape with a bounded `npm install --global @anthropic-ai/claude-code` attempt. It records before/after versions plus npm and provider update evidence in `<TaskRepository>/logs/cli-self-heal.jsonl`; a truly absent package is never installed automatically. The deeper binary/stub healer remains the legacy rollback path described below.
