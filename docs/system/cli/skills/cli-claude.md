...
2. **Windows npm shim quirk.** The npm-installed binary is `node_modules/@anthropic-ai/claude-code/bin/claude.exe`. An interrupted update can leave the package installed while its global `claude`/`claude.cmd` shims vanish, breaking `claude --version`. The local capability coordinator detects that exact shape, runs one bounded `npm install --global @anthropic-ai/claude-code` repair per hour, journals version and npm-activity evidence, and shows the receipt in Execution Hosts. A true uninstall remains an explicit operator action.
