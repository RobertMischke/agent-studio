...
2. **Windows npm shim quirk.** The npm-installed binary is `node_modules/@anthropic-ai/claude-code/bin/claude.exe`. An interrupted npm postinstall or updater can leave the package present while the callable `claude.cmd`/`claude.exe` shim is absent, breaking `claude --version`. The local capability coordinator detects that narrow state and runs `npm install --global @anthropic-ai/claude-code` at most once per hour. It journals package/CLI versions and nearby npm/update file activity in `<workspace>/logs/cli-repairs.jsonl`; the status bar shows a calm repair receipt and alarms only on failure. A true uninstall is observation-only. See `supported-clis.md` section 2.11.
