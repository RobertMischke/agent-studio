...
the global `node_modules` tree but every Windows-launchable command shim is
missing. A stranded PowerShell or extensionless shell shim does not make the
backend probe launchable. Only this package-present shape is repaired
automatically with `npm install --global` for Claude or Codex. The coordinator
permits one attempt per CLI per hour, persists the
...
the read-only probe proves package-present/missing-launchable-shim state, and it also
