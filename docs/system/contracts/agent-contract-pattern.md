...
2. An entry in [docs/system/contracts/loop-inventory.md](./loop-inventory.md) if the command can re-enter a loop.
...
- [docs/system/contracts/agent-task.md](agent-task.md) is the on-disk task lifecycle contract every CLI must respect. Different layer; the contract pattern is about *agent invocations*, the task contract is about *job folders*.
- [docs/system/contracts/loop-inventory.md](./loop-inventory.md) is the registry that this pattern depends on.
