...
- **Action when budget exhausted:** Keep the CLI unavailable and skip the repair until the hour expires. Only a currently unresolved failed attempt raises an operator alarm. A successful repair or later healthy probe removes the provider from `RunnerStatus.CliRepairs`, so a healthy fleet consumes no UI space.
...
- **Last fired:** 2026-08-13 and 2026-08-18 were manually repaired before this loop existed. Both incidents had the Claude npm package present with all global shims absent; the second repair moved the observed version from 2.1.231 to 2.1.234. On 2026-08-31 the Codex force-relink attempt failed because an unvalidated npm launcher resolved `npm-cli.js` under the backend working directory; an operator repair restored the CLI out of band.
- **Notes:** The journal is `<TaskRepository>/logs/cli-self-heal.jsonl` (or `runtime/cli-self-heal.jsonl` without a task repository). Each attempt captures the package/shim state, selected install or force-relink action, last observed/package version, post-repair CLI version, typed npm outcome, npm exit and redacted output, package metadata, and nearby npm debug-log activity. npm resolution prefers the launcher shipped beside the active Node executable, then validates fallback candidates with `npm --version` from an explicit temporary working directory. No validated candidate produces `npm-unavailable` without attempting installation. Success requires both a restored `.cmd` shim and a successful CLI `--version` probe. A later healthy probe appends a terminal `healthy` row so restart rehydration cannot revive the old failure. Custom executable paths and present-but-broken command shims are outside this automatic repair policy.
