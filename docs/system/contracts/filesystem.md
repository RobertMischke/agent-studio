...
- `autoDispatch` - optional queue-depth automation override. `false` prevents automatic `0-backlog` to `2-ready` promotion and automatic Epic decomposition, while `true` or an absent field uses normal queue-depth policy. Manual lane moves and manual starts are unaffected. Set it through task creation or `PUT /api/tasks/{id}/auto-dispatch`; a JSON `null` restores inherited policy.
