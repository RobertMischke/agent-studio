...
```json
{
  ...
  "creationProvenance": {
    ...
    "goalId": "product-goal",
    "goalKey": "AGT-2201",
    "contextKey": "project:agent-taskboard",
...
- `creationProvenance` - server-authored origin for a card materialized by Epic goal decomposition. `initiator` is `orchestrator` for planning-run children and `operator` for deterministic API-created children; `method` is `goal-decomposition`. `goalId` and optional `goalKey` identify the owning Epic, `contextKey` identifies the planning orchestrator context when available, `purpose` is `delivery` or `verification`, and `createdAt` is the server's UTC creation time. Ordinary direct-created and legacy tasks omit the object. Agents and clients must not infer this provenance from titles or write it through the general task-create request.
- `references` - structured cross-references to other tasks by their stable display key (`{ "dependsOn": ["CAR-3"], "relatedTo": [], "blockedBy": [], "supersedes": [] }`), written through `PUT /api/tasks/{id}/references`. Keys are project-unique via the project short code, so they resolve across projects. **`dependsOn` is the "waits-on" relation (AGT-2029) and is scheduler-load-bearing**: a `2-ready` card whose `dependsOn` targets have not all reached `6-completed`/`7-archive` is held back from auto-pickup (it stays visibly "waiting" on the board, never a silent skip) and becomes pickable once they are fulfilled. Fulfillment resolution is cross-project and archive-inclusive. A `dependsOn` cycle is a configuration error: it is reported (structured `waits-on-cycle` log + an error chip on the card) and the card is skipped so the runner never deadlocks. Writing an unknown key is allowed (a warning, not a hard failure) because the referenced task may be created later. The board card renders a state-aware, clickable dependency chip and the task detail shows both directions (waits-on / blocked-by-me). Only `references` is persisted; the task read endpoints (`GET /api/tasks`, `/api/tasks/grouped`, `/api/tasks/{id}`) additionally fold on a derived, non-persisted `waitsOn` object — `{ "items": [{ "key", "resolved", "fulfilled", "targetJobId", "targetTitle", "targetState", "targetWatchPath" }], "blocked", "cycleDetected" }` — computed per request from `dependsOn` against an archive-inclusive, whole-workspace index (`WaitsOnEvaluator`). It is absent/null on cards without `dependsOn` edges and drives the chip; the FE never recomputes fulfillment client-side.
