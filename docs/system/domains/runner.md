...
- `backend/Features/Runner/RunTimeline.cs`: additive projection from durable
  ...
  projection applies the same rule before preferring a terminal RunAttempt,
  `agent_run_finished`, or the last bounded CLI activity. A terminal legacy row with no usable evidence is
