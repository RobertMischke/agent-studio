...
- `backend/Features/Cli/Execution/CliRepairGate.cs` /
  ...
  `NpmShimHealer` pass to one attempt per host per hour under a lock (so
  concurrent callers cannot both pass the check-then-act race), and journals
  every real attempt - success or failure, with the `package.json` version
  read immediately before and after - to `<workspace>/logs/cli-repairs.jsonl`.
