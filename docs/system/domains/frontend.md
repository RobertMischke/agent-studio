...
- The Execution Hosts destination at `#/workspace/settings/execution-hosts`
  ...
  by default, and narrow tables replace complete action buttons with a
  text-only overflow menu. A persisted machine disclosure opens to one compact
  line per section before revealing identity, connection, capability,
  authentication, capacity, lease, project-access, and telemetry internals.
