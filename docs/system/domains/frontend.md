...
- Project Settings owns the project-dedicated execution assignment. The
  ...
  cannot be confused. Fresh runner heartbeat telemetry drives the status bar's
  remote coding and review utilization (`busy/total`), summed across connected
  hosts; host names, host count, and per-host utilization stay in the tooltip.
  Board leases remain an independent consistency check instead of the primary
  utilization figure. A disconnected, expired, or recovering location remains
  an acute orphan candidate. These consumers reuse the grouped-board,
  execution-location, and execution-host snapshots and do not add another
  polling path. The historical target is the
