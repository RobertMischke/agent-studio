...
- Project Settings owns the project-dedicated execution assignment. The
  ...
  execution-host capability snapshots. Its tooltip separates Coding and Review
  occupancy, and a physical host's load is counted once, so elevated load with
  active reviews is not presented as an unexplained-load hint. The compact
  label also includes durable Review active/waiting counts. Waiting work with
  zero active reviews is an amber attention state with an explicit consistency
  hint rather than a normal `no runners` state. A disconnected,
