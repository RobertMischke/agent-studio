...
- Project Settings owns the project-dedicated execution assignment. The
  ...
  execution-host capability snapshots plus the durable waiting count from
  Review queue telemetry. Its tooltip separates Coding and Review occupancy,
  and a physical host's load is counted once, so elevated load with active
  reviews is not presented as an unexplained-load hint. Waiting Review work with
  zero active Review slots is an amber attention state with an explicit
  consistency hint, never a normal `0` or `no runners` state. A disconnected,
