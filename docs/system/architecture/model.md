...
| `sourceRefs` | optional | Repository-relative paths or fragment links to authoritative docs and ADRs (for example `docs/system/architecture/decisions/adr-archive.md#adr-0024`). |
...
1. **In the watched project's repository**, at `architecture/<modelId>.md` (or a project-chosen sibling, for example `docs/system/architecture/<modelId>.md`). The model is part of the project's documentation and travels with its history. This is the default.
...
elements:
  - elementId: frontend-shell
    ...
    sourceRefs:
      - frontend/AGENTS.md
      - docs/quality/design-principles.md
    relevantTests:
      - frontend/e2e/**
    runtimeSignals:
  ...
  - elementId: task-access
    ...
    allowedDependencies:
      - in-memory-store
    sourceRefs:
      - docs/system/architecture/decisions/adr-archive.md#adr-0024
    relevantTests:
