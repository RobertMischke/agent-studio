...
Mockup: [mockups/task-processing-pipeline/pipeline-editor.md](../../../../concepts/task-processing-pipeline/pipeline-editor.md).
...
This renders on the unified `timeline.jsonl` ledger ([ADR-0049](../adr-archive.md#adr-0049), ASS-560), which already carries `pre_step_started` / `pre_step_finished` / `post_step_started` / `post_step_finished` event kinds. The pipeline is the backbone of the timeline the operator has repeatedly asked for.

Mockup: [mockups/task-processing-pipeline/task-timeline.md](../../../../concepts/task-processing-pipeline/task-timeline.md).
...
- [ADR-0023](../adr-archive.md#adr-0023): "A database engine. No SQLite, no LiteDB, no EF, no embedded server." The cross-cutting data layer is file-backed JSONL + an in-memory `InMemoryStore<T>` projection.
- [ADR-0024](../adr-archive.md#adr-0024): "A database engine. No SQL, no LiteDB, no EF. Files plus an in-memory index."
...
|---|---|
| Pipeline foundation | Extends [ADR-0045](../adr-archive.md#adr-0045) `TaskPipeline` / `PipelineStep` / `PipelineExecutionRecord`. Adds the envelope fields + `StepType` + per-project versioned definitions. |
| Timeline render | Renders on [ADR-0049](../adr-archive.md#adr-0049) `timeline.jsonl` (ASS-560). The `*_step_started/finished` kinds already exist. |
| Completion-loop | The loop (ASS-566) **wraps** the pipeline: each task attempt runs the full pipeline; `orchestratorReaction: review` steps that return `reopen` reissue the core work and tick the loop budget. Registered in [loop-inventory.md](../../../contracts/loop-inventory.md). |
| Orchestrator-review safety | `review` steps run through [ADR-0032](../adr-archive.md#adr-0032): agent classifies (schema-validated output), rule engine decides halt/reopen/escalate. |
| Per-step model | Reuses the shared CLI+model selector (ASS-544/562). No new picker. |
| Editor mutations | [ADR-0046](../adr-archive.md#adr-0046) optimistic-UI; reorder reuses the kanban drag-snapshot revert pattern. |
...
| Job-folder writes | Definitions live under `.metadata/`; per-job `step-runs.jsonl` is written through the Task Access layer's append path ([ADR-0024](../adr-archive.md#adr-0024)), never a direct folder write. |
...
- UX mockups: [pipeline editor](../../../../concepts/task-processing-pipeline/pipeline-editor.md) and [task timeline](../../../../concepts/task-processing-pipeline/task-timeline.md), indexed at [mockups/task-processing-pipeline/README.md](../../../../concepts/task-processing-pipeline/README.md).
