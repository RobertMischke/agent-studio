...
2. An entry in [docs/contracts/loop-inventory.md](./loop-inventory.md) if the command can re-enter a loop.
...
[docs/contracts/loop-inventory.md](./loop-inventory.md) is the registry of every place in the codebase where work can re-enter itself: retry, requeue, re-trigger, replay. Each entry names its kind (Pre-Guard / Post-Guard), the code anchor, the budget constant, and the breaker test.
...
- [ADR-0002](../architecture/decisions/adr-archive.md) (deterministic orchestration over prompt trust) is the parent decision; this pattern is the next layer up.
- [ADR-0017](../architecture/decisions/adr-archive.md) (supervisor advice-first) is the sibling that bounded *one* kind of agent (the supervisor) the same way; ADR-0032 generalises the rule.
- [docs/contracts/agent-task.md](agent-task.md) is the on-disk task lifecycle contract every CLI must respect. Different layer; the contract pattern is about *agent invocations*, the task contract is about *job folders*.
- [docs/contracts/loop-inventory.md](./loop-inventory.md) is the registry that this pattern depends on.
