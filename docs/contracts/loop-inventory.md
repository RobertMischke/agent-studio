...
- **Re-entry trigger:** A CLI run ends with a transient process abort (`RunIssueKind.WatchdogTimeout` -> `OutcomeActionKind.NotifyUserAndStop`). Instead of dead-ending the task in `5-human-review`, the completion loop re-spawns the same job. Fires only when the LLM abort-review step (default-OFF per project) did NOT handle the run, so for most projects this is the only completion loop.
- **Budget:** `CompletionRetriggerDecider.DefaultBudget` (default 2 automatic re-triggers per job). Counter `_completionRetriggerUsed` is per `jobId`.
