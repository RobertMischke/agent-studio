...
| delivered | AGT-2229 / AGT-2262 | Canonical Remote review gates run as immutable tool commands inside claimed and fenced ReviewAttempts. The SSH dispatch, host semaphore, `gate-work` convention, alias activity projection, and local fallback were removed after replacement. | [decision history](decision-history.md) |
...
| delivered | AGT-2222 T1+T2 | Derived admission, LLM budget, and latency metric. Its temporary SSH remote-gate shortcut reached a 2 minute 11 second full suite and is now retired. | [decision history](decision-history.md) |
