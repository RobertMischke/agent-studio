...
- **A stale validator path (bonus finding).** `scripts/visual-docs/validate.mjs`
  ...
  `docs/quality/visual-features/manifest.json` (its own `sourceOfTruth` field says so).
...
- **T6 (cleanup, decision-independent) - Fix the visual-docs validator path.**
  ...
  does not exist; point it at `docs/quality/visual-features/manifest.json`. Small,
