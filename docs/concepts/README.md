...
| [batch-gate-mechanics.md](batch-gate-mechanics.md) | Target mechanism (decision-pending, unimplemented): batching eligible deliveries into one fenced candidate, one full suite run, per-member evidence, and bounded halving for red isolation. | [`docs/operations/batch-gate-concept/index.html`](../operations/batch-gate-concept/index.html) |
| [remote-gate-target-architecture.md](remote-gate-target-architecture.md) | Target architecture (decision-pending, unimplemented): `GateSubject`/`GateAttempt`/`GateLease` claim-based model replacing the direct-SSH build/test gate bridge. | [`docs/operations/remote-gate-zielbild/index.html`](../operations/remote-gate-zielbild/index.html) |
| [rebase-merge-invariants.md](rebase-merge-invariants.md) | Attribution invariants and stage-specific Git policy for integration (mostly implemented current state), plus the still-open deterministic bounce-handler proposal. | [`docs/operations/rebase-merge-and-steering/index.html`](../operations/rebase-merge-and-steering/index.html) |
| [telemetry-layer.md](telemetry-layer.md) | Proposal (decision-pending, unimplemented): bridging Product Runtime Observability and the Agent Message Bus with a normalized `OrgTelemetryEvent/v1` envelope and a bounded orchestrator action contract. | [`docs/operations/telemetry-layer/index.html`](../operations/telemetry-layer/index.html) |
