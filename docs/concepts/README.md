...
|---|---|---|
| [batch-gate-concept.md](batch-gate-concept.md) | Proposed (not yet approved) mechanism for assembling every eligible pending delivery onto one fenced temporary branch, running one full suite, and publishing an exact green tip with per-member evidence and bounded red-path halving. | [`docs/operations/batch-gate-concept/index.html`](../operations/batch-gate-concept/index.html) |
...
| [telemetry-layer.md](telemetry-layer.md) | Proposed (not yet approved) organization-wide telemetry architecture: privacy-bounded event envelope, log-level mapping, hybrid file-plus-push bus delivery contract, and clustered-signal-to-bounded-orchestrator-action mapping. | [`docs/operations/telemetry-layer/index.html`](../operations/telemetry-layer/index.html) |
