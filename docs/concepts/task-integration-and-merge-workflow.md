...
The exact ordered merge-first / `rerere` / cardinality-preserving-rebase sequence behind the paragraph above, and the single-round `AgentRoundRequired` automatic recovery it feeds (`IntegrationAgentRoundService`), are documented in detail in [Rebase, merge, and integration attribution invariants](rebase-merge-invariants.md) (extracted from the [rebase-merge-and-steering dossier](../operations/rebase-merge-and-steering/index.html), AGT-2662) rather than duplicated here.
...
- `docs/concepts/rebase-merge-invariants.md` (AGT-2662) - the exact merge-first / `rerere` / cardinality-preserving-rebase sequence and the `AgentRoundRequired` recovery round, extracted from the `rebase-merge-and-steering` dossier, which retains the still decision-pending generalizations (auto-requeue rail, guardian escalation, Batch Gate interaction).
