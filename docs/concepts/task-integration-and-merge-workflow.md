...
- Acceptance rail: acceptance never starts integration. `AcceptanceRailHostedService` runs every three minutes by default and calls `TaskTransitionService.MoveAsync` only for an ordinary Human Review coding card whose batched Git projection is already `integrated`. The transition repeats current-attempt and Git validation at the mutation boundary, so a stale sweep cannot complete a non-integrated card. Concept cards and operator holds remain parked. `/api/pipeline/acceptance-rail-status` exposes Human Review and Escalated depth plus last-run/action time.
...
- Commit: `Merged`, `MergedAfterRebase`, and `AlreadyMerged` make the Git-derived projection `integrated`; the rail may then perform only the lane move to `6-completed`. `NoTaskBranch`, `Conflict`, `GateFailed`, and `Error` remain in Human Review with their typed evidence. A configured pull-request handoff also remains there until target-branch membership is real. The legacy accepted-integration worker and backstop retain compatibility recovery for old durable `integrating` transactions, but no new acceptance request creates one.
...
- Restart recovery: `AcceptanceRailHostedService` is the normal deterministic terminal owner and starts immediately after backend startup before continuing on its bounded timer. `TaskIntegrationRecoveryService` stores the conflict counter and any in-progress recovery fingerprint beside the task logs, so restart resumes a partially applied recovery without duplicating the prompt or resetting the budget. The older `AcceptedIntegrationBackstopHostedService` remains only for compatibility transactions created before the integration-before-acceptance cutover.
...
- Conflict recovery: only a delivery with a typed rebase-recoverable
  `conflict-skipped` failure is eligible. The rail and
  `POST /api/tasks/{id}/integration/rebase` share
  `TaskIntegrationRecoveryService`, which appends an idempotent `## STEER`,
  persists the pending intent, supersedes the failed delivery, and moves the
  card to the top of Ready. The default automatic limit is five conflict
  requeues. Exhaustion reaches `5e-escalated` with category
  `integration-recovery-exhausted` instead of looping again.
