...
was disabled, and every worktree was removed after the read-only experiment.
...
| --- | --- | --- |
| Worktree / branch | No run worktree; a behind-base delivery uses one disposable detached recovery worktree | Isolated worktree on `task/<id>` |
...
| Merge command / history | If behind: isolated conflict-free rebase, then `git merge --no-ff`; rewritten SHAs are linked in `commits[]` | rebase + `git merge --ff-only` (linear, rewritten SHAs) |
| Serialization | Implicit (single active slot) | `_integrateLock` semaphore + cross-runner integration lease |
| Conflict handling | Mechanical replay first; on a real textual conflict abort, list files, and offer focused operator recovery | Preserve and escalate to a managed resolver; block teardown if unresolved |
