...
- Trigger: a task moving `5-human-review -> 6-completed` (operator accepts a done-green task). `TaskTransitionService.MoveAsync` stamps `integrationpending` and writes an immutable request to `AcceptedIntegrationQueue`; the HTTP request does not wait for merge or build.
