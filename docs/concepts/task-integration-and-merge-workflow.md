...
- Execution: both the immediate coordinator and `AcceptedIntegrationWorker` call `MergeIntoDevelopRunner.Run` (`backend/Features/Pipeline/MergeIntoDevelopRunner.cs`). Immediately before any merge or release gate, the runner fetches the configured integration branch and fast-forwards a stale local branch. It leaves a local-ahead branch intact and reports a divergent branch without overwriting either tip. When the delivery is behind the synchronized target, Git replays it with `rebase --rebase-merges` in a disposable detached worktree with `rerere` and autostash disabled. Only a fully conflict-free replay with an unambiguous one-to-one commit mapping may proceed to the normal merge and gate. A textual conflict aborts the replay and reports the unmerged file list. Cleanup must remove both the temporary directory and its Git worktree registration before the target repository is mutated.
...
## Merge-first sequence and the ambiguous-attribution recovery round

`GitService.MergeRefIntoIntegration` (`backend/Features/Git/GitService.cs`) does not pick a single Git operation when a reviewed delivery is behind the synchronized integration branch; it works through an ordered sequence, and each stage exists to keep delivery SHAs reachable for as long as possible before Git is allowed to rewrite them.

1. **Direct merge first.** The runner attempts `git merge --no-ff` of the delivery branch (or fenced Remote ref) into the synchronized integration branch. When Git can merge the two histories without a conflict, every original delivery commit remains an ancestor of the integration branch and the merge commit is the only new object - the common case, and the reason `commits[]` ancestry can prove integration directly.
2. **Mechanical three-way merge with `rerere`.** A direct conflict does not fall straight to a rebase. The runner first retries the merge with `rerere.enabled=true` / `rerere.autoupdate=true` recorded resolutions, still building a `--no-ff` merge commit. This still preserves every original delivery SHA; it only changes how conflicted hunks are resolved.
3. **Cardinality-preserving isolated rebase, as a last resort.** Only if the three-way merge still conflicts does the runner replay the delivery with `git rebase --rebase-merges` inside a disposable detached worktree (`rerere` and autostash disabled there), as described above. The replay is accepted only when fully conflict-free and one-to-one (same commit count; no split, drop, or squash). A successful replay reports `MergedAfterRebase`, and the caller persists the old-to-new commit map through `TaskMutationService.RecordMechanicalRebaseOnFolder` before anything is pushed; if that attribution write fails, the merge is rolled back and nothing reaches origin.
4. **`AgentRoundRequired` when the mapping is ambiguous.** If the rebase replay changes commit cardinality or otherwise cannot produce a clean map, the integrator refuses to guess and returns `MergeIntoIntegrationOutcome.AgentRoundRequired` instead of a bare conflict.

That fourth outcome feeds a dedicated, already-implemented recovery path distinct from the operator-triggered "Rebase & retry" action described above: `IntegrationAgentRoundService` (`backend/Features/Pipeline/IntegrationAgentRoundService.cs`). Its `RemoteIntegrationContinuationPolicy` is a pure eligibility check permitting exactly one automatic Steer round per operator review epoch (`MaxAutomaticAgentRounds = 1`, epoch from `OperatorReviewRequeueService.ReadEpoch`). When eligible, the service saves a pending Steer intent naming the exact sequence that was tried, marks the ambiguous delivery superseded (`TaskCommitSupersession.PendingAttempt`), and promotes the card to the top of Ready - through the existing mutation and state-machine services, with no model call needed to decide eligibility. A second `AgentRoundRequired` result within the same review epoch is left for Human Review instead of retried; every attempt is recorded on the task timeline (`IntegrationRecoveryQueued` / `IntegrationFailed`).

The distinction matters operationally: `POST /api/tasks/{id}/integration/rebase` handles a reviewed `conflict-skipped` card generically and requires an operator click, while `IntegrationAgentRoundService` fires automatically and only when SHA attribution itself would otherwise become ambiguous - before the card ever reaches Human Review. Both ultimately queue a Steer-and-supersede round through the same underlying services, but they trigger on different signals at different lifecycle points, and only the second is unconditioned on operator action.

Source: [Rebase, merge, and bounce steering decision dossier](../operations/rebase-merge-and-steering/index.html) (AGT-2662). The dossier's further generalizations (a platform-side auto-requeue rail for every `conflict-skipped` bounce, guardian escalation, Batch Gate interaction) remain decision-pending and unimplemented; only the merge-first sequence and the single-round `AgentRoundRequired` recovery above are shipped.
...
- `docs/operations/rebase-merge-and-steering/index.html` (AGT-2662) - the decision dossier behind the merge-first sequence and `AgentRoundRequired` recovery round; also covers the still decision-pending generalizations (auto-requeue rail, guardian escalation, Batch Gate interaction).
