...
implementation exists and the design itself has not been formally approved —
the source dossier frames every decision (D1-D6) as open, pending operator
sight review. This page documents the settled *shape* of the recommendation
from the
[remote-gate-zielbild dossier](../operations/remote-gate-zielbild/index.html)
(`AGT-W18`, sourceTaskKey `AGT-2369`) so the target model is discoverable
outside that dossier's decision narrative. The dossier remains the
system-of-record for the open decisions and rollout/cutover plan; do not
treat anything here as already running.
...
- **GateLease** — lease ID, attempt ID, executor/instance/host, fence,
  ...
  [lease/fence/epoch conventions](distributed-agent-studio-target-architecture.md#lease-fence-and-authority-epoch-mechanics)
...
with, and does not replace, [Batch Gate mechanics](batch-gate-mechanics.md):
a batch's single full-suite run is a natural GateSubject, and the batch
coordinator lease and project ref-mutation lease are additional authority
layered on top of a GateLease, not a competing mechanism.
...
- 2026-08-18 (AGT-2671): extracted from the remote-gate-zielbild dossier as
  part of an operator-mandated dossier curation pass. Design only, not
