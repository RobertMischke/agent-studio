...
> **Naming decision, 9 August 2026:** The user-facing artifact is **Dossier**
> (plural: **Dossiers**). Existing technical contracts remain unchanged,
> including `workbench.json`, `Workbench*` code identifiers, documentation
> slugs, and the `/workbenches` API routes. No `/dossiers` route alias is part
> of this decision.
...
examples, and converge before it becomes production work. Pipeline Dossiers,
...
There is no Dossier registry file. The physical folders (each with a
...
- Dossier id, title, path, branch, revision, status, and phase;
