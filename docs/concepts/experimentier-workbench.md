...
| Iterate | `active / testing` | Change the files through normal Git-aware editing; use the Dossier's dedicated orchestrator context. |
...
orchestrator. The right column is the existing resizable Orchestrator Chat
described in [Persistent Orchestrator Chat](./orchestrator-chat.md) and
[Orchestrator in-app](orchestrator-in-app.md).

The canonical conversation context is `dossier:<PROJ-ID>/<DOSSIER-ID>`. It is
created empty on first access and resumes the same retained transcript when the
operator returns. The Dossier context receives a bounded evidence attachment
containing:
...
This preserves one project orchestrator while giving each Dossier its own
durable conversation. Dossier open/close and decision events can appear only
in that Dossier context as compact transcript anchors. Every anchored turn
records the Dossier id and entrypoint fingerprint or Git revision it actually
saw. Project, task, Dossier, and global monitoring histories remain strictly
separate. Route-derived selection prevents a previously active board or watcher
stream from appearing in the Dossier transcript.
...
| **WB-3: Canonical Dossier chat** | M/L | Reuse the chat side sheet with a dedicated Dossier context key and attach the bounded Dossier digest plus current selection. Add compact open/close anchors. | First access is empty, return access resumes, the header identifies the Dossier, and no project, task, or monitoring history leaks into it; source editing remains out of scope. |
...
- the context model now uses a dedicated retained Dossier transcript while
  preserving the project orchestrator as the execution owner;
