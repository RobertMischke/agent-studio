...
The project composer keeps the active tab as automatic context and renders
explicit additions in the library-owned context-attachment row as removable
reference chips. One Plus button in the footer opens a compact menu containing
only **Add context**; that action opens a project-bound picker with the current
surface first, followed by Tasks, Wiki and Dossiers, Files, and Commits. The
footer does not repeat the active location. The composer has no image picker,
paste, or drop staging and no upper routing toolbar; the model selector and
Send action stay in its footer. Search results
...
preview, or other tab surface. The side sheet passes that value to CAC's
first-class `composerContext` input but suppresses the duplicate visible
location string. The same value drives the automatic-context suggestion in the
Add context picker, and CAC never re-derives navigation state. Changing tabs
updates the value in place, so the composer stays mounted and preserves its
draft. Studio projects only the Plus action into the standard footer, with no
task-creation buttons or parallel footer workflow.
