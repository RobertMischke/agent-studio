...
reference chips. The single secondary-action Plus button opens a compact menu
whose only v1 action, **Add context**, opens the project-bound picker with the
current tab first, followed by Tasks, Wiki and Dossiers, Files, and Commits. The composer has no image picker,
paste, or drop staging, no upper routing toolbar, and no duplicate location
line; Plus, the model selector, and Send stay in its footer. Search results
...
preview, or other tab surface. The side sheet uses that value for the panel
header, the picker's current-surface recommendation, and the context envelope;
it does not duplicate the location in CAC's compact footer. CAC never
re-derives navigation state. Changing tabs updates the value in place, so the
composer stays mounted and preserves its draft. Studio projects only the Plus
action and does not add task-creation buttons or a parallel footer workflow.
