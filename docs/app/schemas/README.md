...
- `wiki-page-lifecycle.schema.json` - shared lifecycle contract for designs, concepts, explorations, and Workbenches. Markdown uses leading frontmatter; HTML Workbenches use the same lifecycle fields in `workbench.json`. Companion sidecars never duplicate this state.
