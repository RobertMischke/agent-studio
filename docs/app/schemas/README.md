...
- `quality-analysis-policy.v1.schema.json` - the repository-owned, versioned override for standard Quality Studio analysis axes. Read only from `.quality/agent-studio-pipeline.json` in the reviewed repository.
- `quality-studio-analysis-evidence.v1.schema.json` - one durable result from a named Quality Studio analysis step, including package and source provenance, applied rule ids, findings, and sensor provenance.
