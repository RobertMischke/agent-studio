...
- `quality-studio-analysis-evidence.schema.json` - one durable result artifact from an in-process Quality Studio analysis step, including the package version, named analysis, analysed paths, status, and findings with stable QS rule ids. Stored under a job's `results/quality-studio/` folder.
