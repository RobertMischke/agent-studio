...
- `cli-repair.schema.json` - one attempt or terminal row in `<workspace>/logs/cli-repairs.jsonl`, recording the bounded local Windows npm-shim repair, CLI/package versions before and after, and recent redacted npm activity evidence.
