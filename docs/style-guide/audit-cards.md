...
- Some cards have a leading accent stripe (task-status-card, drift-card-warn); others have a tinted background (running job-card via `--studio-bg-running`).
...
| `.task-status-card`       | `components/task-status-card/`                                       | `--studio-bg-elevated`       | accent left-stripe      | 12px      | Reusable status card (shared)        |
...
| `.drift-card`             | drift section / overview                                             | `--studio-bg-elevated`       | severity left-stripe    | 12px      | Drift card with status accent        |
| `.steer-card`             | orchestrator side sheet                                              | `--studio-bg-elevated`       | accent left-stripe      | 12px      | "Steer" message card                 |
...
Twelve+ card classes, **all** built on `--studio-bg-elevated` + a border + 8-12px padding. The recipe is identical; the deltas are the **accent treatment** (none / left-stripe / running-tint) and the **hover behaviour** (lift / flat).
...
3. **`<app-card variant="accent">`** — left-stripe (or top-stripe) with a per-prop accent token. Covers task-status-card, drift-card, steer-card.
