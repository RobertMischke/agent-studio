...
A future `size="lg"` is intentionally not shipped today. Per [`docs/design-system.md`](../design-system.md) and the operator preference for information-dense surfaces, a "spread-out" variant has no current use case. Add it only when a real dialog wants it.
