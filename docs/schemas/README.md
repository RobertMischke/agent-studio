...
- `drift-report.schema.json` - one project-level drift analysis report with scores, dimensions, evidence references, optional architecture-element projection, and follow-up task suggestions. Stored at `<workspace>/logs/drift/<project>/<reportId>.{md,json}`. Prose contract in [`docs/drift-reports.md`](../drift-reports.md).
...
- `product-runtime-event.schema.json` - one structured event emitted by the software the agents are building. Append-only JSONL under `<job>/logs/runtime/<date>.jsonl` for job-scoped runs and `<project>/logs/runtime/<date>.jsonl` for project-scoped runs. Separate stream from the Agent Message Bus; joined by reference, never mixed. Prose contract in [`docs/product-runtime-observability.md`](../product-runtime-observability.md).
...
- `update-run-verification.schema.json` - one row per phase-6 verification check (healthz-stable, runner-status, jobs-grouped, clients, cli-quota, db-touch). Appended to `<RunsDirectory>/<runId>/verification.jsonl`. ADR-0031.
