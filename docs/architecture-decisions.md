...
## ADR-0011 - CLI-process spawn boundary: prefer the underlying .exe, gate live tests behind RUN_CLI_INTEGRATION (2026-05-03)

**Decision.** When the runner spawns a Node-based CLI on Windows (Claude / Codex / Gemini), it always invokes the underlying `.exe` (or `node.exe <bundle>.js` for the pure-JS CLIs) and never the npm-installed `.CMD` shim - even though Windows would happily resolve the `.CMD` via `PATHEXT`. The shim path forces `cmd.exe` wrapping and that wrapping was the prime suspect in a recurring "agent goes silent after the `system/init` frame" hang. We also keep an opt-in integration-test suite at [`backend.Tests/CliSpawnIntegrationTests.cs`](../backend.Tests/CliSpawnIntegrationTests.cs) that spawns each CLI for real with `--model claude-haiku-4-5` (cheap), gated behind `RUN_CLI_INTEGRATION=1` so default unit-test runs stay free and fast. Each probe pins one shape of the spawn (direct `.exe` + pipe stdin, `.CMD` shim with fat prompt, `ClaudeCliService.StartAsync` with a realistic fat prompt + `agent-rules` system-prompt-file, sequential kill+restart) so future regressions show up at the test level instead of as hung jobs in production.

**Context.** Live runs on `claude.exe` (npm-installed via the `claude.CMD` shim) intermittently produced only the first `system/init` stream-json frame, then went silent until the watchdog killed the process at 124s. Direct shell invocation with the same args worked in 4s. The .NET runner did `Process.Start("claude")` which Windows resolves to `claude.CMD`, which forces a `cmd.exe /c "..."` wrap. The hypothesis - "cmd.exe wrapping breaks redirected-stdin pipe inheritance for Node child processes" - turned out to NOT reproduce in isolated tests (a fat-prompt `.CMD` invocation streams correctly), but pointing the runner at the underlying `.exe` is the cleaner contract anyway and removes one variable from any future hang investigation. The decision pins both halves: the production code path and the regression matrix.

The investigation also surfaced a parallel question from the user: "is this a WSL2 vs Windows thing? Should we make WSL2 a requirement?" The answer is no. Node's stdout block-buffers on pipes on every OS (Linux/macOS/Windows) - WSL2 would not fix the buffering hypothesis, and the practical Windows-specific complication (`.CMD` shim wrapping in `cmd.exe`) is solved by addressing it directly. WSL2 as a requirement would force a Linux variant of every CLI, lose Windows-native authentication paths (Copilot CLI's Windows TUI), and not change the underlying mechanics.

**Non-goals.**
- Pseudo-terminal (PTY) spawn for headless CLI runs. We tried it; Porta.Pty + winpty does not co-exist with `claude -p` (the CLI sees `stdin = TTY` and exits with code 1). PTY is reserved for genuinely interactive CLIs - Copilot's `PtySession` is the existing example. The base-class `SpawnChildAsync` virtual hook stays in place for that use case but is NOT exercised by the headless path.
- Forcing WSL2 / Linux as the runtime. Cross-platform parity is fine but this product targets Windows-native development (the user's seat) and the watched targets are Windows projects.
- Running the integration matrix in default `dotnet test`. They burn quota and take seconds per test; CI runs them on a nightly opt-in job, agents run them when investigating spawn issues (`RUN_CLI_INTEGRATION=1 dotnet test --filter CliSpawnIntegrationTests`).
- Mocking the CLI in these tests. The whole point is to drive the live binary so future package updates / Windows patches / CLI version bumps fail loudly. Pure-unit coverage stays in [`ClaudeCliServiceTests`](../backend.Tests/ClaudeCliServiceTests.cs) (frame parsing) and [`OrchestratorRunnerTests`](../backend.Tests/OrchestratorRunnerTests.cs) (one-shot JSON envelope).
...
- Testing every concurrency permutation. The stress test pins three sequential kill+restart cycles - enough to catch obvious accumulated-state bugs - not an exhaustive interleaving matrix.

**Reasoning style.** Test what we can spawn cheaply, document what the test pins, and treat the live CLI as the source of truth. The four probes were chosen to triangulate the hang root cause: (1) does the direct `.exe` path stream past init? (2) does the `.CMD` path stream when given the realistic prompt scale? (3) does the production `ClaudeCliService.StartAsync` stream end-to-end? (4) does sequential kill+restart accumulate state? Each probe answers one yes/no question, and the assertion message documents what the answer means so the next reader does not have to re-derive the diagnosis from zero.

**Implementation pointers.** [`backend/Services/Cli/ClaudeCliService.cs::ResolveCmdShimToExe`](../backend/Services/Cli/ClaudeCliService.cs) (the npm-shim → `.exe` resolver, called from `BuildStartInfo`); [`backend.Tests/CliSpawnIntegrationTests.cs`](../backend.Tests/CliSpawnIntegrationTests.cs) (the integration matrix, `RUN_CLI_INTEGRATION=1` gate); [`docs/cli-skills/cli-claude.md`](cli-skills/cli-claude.md) (operator-level "what to check when claude hangs" playbook).

**Status.** Accepted. The PTY override that was briefly added during the investigation has been reverted; only the `.CMD → .exe` resolver remains in production code. Codex / Gemini / Copilot smoke probes are queued for the next iteration of the matrix; their CLIs were not implicated in the original hang but parity coverage is desirable.
