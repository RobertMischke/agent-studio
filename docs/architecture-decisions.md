...
## ADR-0037 - Workspace + project identity registry; WatchPaths demoted to bootstrap (2026-05-24)
...
**Current state (2026-05-24).** F45a (read surface + boot-time auto-discovery) shipped. F45b (REST mutations: create / rename / archive / reassign workspace / change color / reorder) is queued. F45c (per-project task-key floor backfill, jobKey-format migration, optional folder restructure under `projects/PROJ-XXX/jobs/<shard>/<slug>/`) is queued. F46 (frontend tree integration) is queued. F47 (this cleanup task) writes this ADR, surfaces the read-only registry in the Settings panel, and documents the WatchPaths demotion in `appsettings.Local.json`; the write-side mutations and the actual WatchPaths removal land with F45b/F45c/F46.

**Status.** Accepted; partially implemented (F45a). Full implementation tracked under F45b / F45c / F46.
