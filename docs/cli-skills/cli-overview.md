...
> **Authoritative contract:** [docs/supported-clis.md](../../docs/supported-clis.md) is the formal "what every supported CLI must provide" document. It is updated in the same PR that touches a CLI integration. This skill complements it with operational/working knowledge.
...
| Adding a new CLI | This skill + [docs/supported-clis.md](../../docs/supported-clis.md) §4 checklist |
...
| Codex | UUID | Parse `payload.id` of the first `session_meta` JSON line in `OnOutputLine`. | `exec resume <uuid> "<prompt>"` (positional, before `--json`) |
...
| `● Session init <uuid> (<model>)` | `message` | Used to capture session UUID via regex. Keep the shape stable. |
| `● Result <subtype>` | `message` | Final-result frame. |
...
|---|---|---|
| Add a new tool marker for an existing CLI | `TransformReadLine` switch in the relevant driver | A new `TransformReadLine_<Tool>MapsToMarkerLine` test in the matching `*CliServiceTests.cs` |
...
| Add a brand-new CLI | [docs/supported-clis.md](../../docs/supported-clis.md) §4 checklist | New `<Cli>CliServiceTests.cs` + new `@billable` E2E `frontend/e2e/<cli>-hello-world.spec.ts` |
