...
> **Contract:** [docs/cli/supported-clis.md §3.2](../supported-clis.md).
...
1. **NOOP on every fresh job** (codex 0.130+): handing the prompt as the
   ...
   provided"). See `docs/cli/investigations/codex-runner-investigation.md` for the forensic
   write-up.
...
- **Don't** revert to positional-argv prompt delivery. `BuildStartInfo_LongPromptKeepsPromptOutOfArgvAndUsesStdin` locks the stdin path; `docs/cli/investigations/codex-runner-investigation.md` records why.
