...
- [design-system-options.md](design-system-options.md) - design-system comparison and the Found Next Workbench recommendation.
...
- [../../research/embedded-chat-integration-2026-05.md](../../research/embedded-chat-integration-2026-05.md) - current working plan that scopes the next slices (host adapter, decision cards, side-sheet adoption, composer toolbar, workbench split, regression suite) on top of the landed bridge.
...
The Angular prototype makes the same direction clickable as a standalone mockup app served with `npm run mockup:chat` on `http://127.0.0.1:4022`. It is intentionally full-screen and self-contained, so production chat remains untouched and the normal dev app cannot accidentally turn into the mockup through a localStorage flag. The newest iteration is the tall workbench: task metadata, split controls, scenario switches, and run metrics sit in a left task rail or status bar so the active work document can use almost the full task height. The prototype now also mirrors more of the existing task-detail product surface: compact task chrome, Complete & Next, an optional Queue module, a richer composer command deck, clickable run markers, and a multi-tab Verbose Debug view. The rail uses inline icons, readable labels in comfortable density, compact icon-only mode, and an explanatory guide modal. The latest cleanup removes duplicated Chat/Git pane switches from the top chrome: document visibility belongs in the task rail, the top chrome carries only run summary plus global Sheet and Queue shortcuts, and the bottom bar opens Queue, Token, Health, Evidence, and Model popovers without stealing height from the active document. Queue selection is also optional: the `Tasks / Queue` activity module can close so documents and review panes get the width. Chat is optional: it is one document tab among Summary, Git changes, Screenshots, and Debug trace. The current prototype now follows the VS-Code-like document model directly: each tab owns the full content for that surface instead of rendering every opened document beside chat. See [angular-prototype.md](angular-prototype.md).

The design-system direction is now explicit: use an internal Found Next Workbench Design System. VS Code supplies the container philosophy, density, icon vocabulary, and source-code reference points; it is not a component library to clone wholesale. Production primitives should be Angular-native, built on Angular CDK and Angular Aria where useful, styled through owned CSS tokens, and backed by Codicons or equivalent local SVG symbols. See [design-system-options.md](design-system-options.md).
