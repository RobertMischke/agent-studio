...
| Angular clickware prototype | `frontend/src/app/components/mockups/next-gen-chat-workbench-prototype.component.ts`, `atp.flag.nextGenChatPrototype` | Interactive shell for reviewing tall workbench layout, left inspector rail, split presets, side sheet, source map, light/dark, density, debug, and mobile collapse |
...
9. Add the task Chat workbench split presets and left inspector rail: Chat only, Result, Git, Preview, Debug, Source; state, token, commit, file, screenshot, warning, duration, task tab, and scenario controls.
10. Add Verbose Debug using the same projected events plus run, token, artifact, and screenshot aggregates.
11. Add Playwright coverage for current layout off, NextGenChat on, VsCodeLayout on, both flags on, light theme, dark theme, mobile, side sheet wide, wait-loop scenario, image lightbox, schema-drift row, workbench Git split, workbench compact density, chat-only preset, Source map, and Verbose Debug.
