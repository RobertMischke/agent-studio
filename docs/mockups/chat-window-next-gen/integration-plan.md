...
This plan explains how the v5 chat mockup should move into the existing app without losing the current product surface. The goal is not to paste the mockup over the app. The goal is to migrate the current Activity Log, project side sheet, run evidence, token indicators, and task controls into one shared chat grammar.
