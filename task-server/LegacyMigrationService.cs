...
public sealed class LegacyMigrationService(TaskServerStore store)
{
            ...
            && reviewAttempts.ValueKind == JsonValueKind.Array)
        {
            foreach (var item in reviewAttempts.EnumerateArray())
            {
                ...
                reviews.Add(new LegacyReviewAuthorityImport(
                    ...
                    ReadDate(item, "createdAt") ?? File.GetLastWriteTimeUtc(path),
                    ReadDate(item, "terminalAt"),
                    ReadReviewOutcome(item, "outcome"),
    ...
    private static string? ReadReviewOutcome(JsonElement item, string property)
    {
        ...
        if (value.ValueKind == JsonValueKind.String)
            ...
            {
                "infrastructurefailure" or "reviewinfra" => "ReviewInfra",
                "productfailure" => "ProductFailure",
                "inconclusive" => "Inconclusive",
                "pass" => "Pass",
                "cancellation" => "Cancellation",
                "superseded" => "Superseded",
                var text => throw new InvalidDataException($"Unknown legacy review outcome '{text}'."),
            };
        throw new InvalidDataException($"Unknown legacy review outcome '{value}'.");
    }
