...
public sealed class LegacyMigrationService(TaskServerStore store)
{
    ...
    public async Task<LegacyMigrationResult> ImportAsync(LegacyMigrationRequest request, string actorId, CancellationToken ct)
    {
        ...
        await store.ImportLegacyBatchAsync(
            scan.MigrationId, request.WorkspaceName, scan.Projects, scan.Authority, actorId, ct);
    ...
    private static async Task<LegacyScan> ScanAsync(string root, bool includeContent, CancellationToken ct)
    {
        ...
        {
            Runners = await ReadRunnerIdentitiesAsync(root, authority, sourceFiles, warnings, ct),
        };
    ...
    {
        var identities = new Dictionary<string, LegacyRunnerImport>(StringComparer.OrdinalIgnoreCase);
        ...
        if (Directory.Exists(identitiesDirectory))
        {
            foreach (var path in Directory.EnumerateFiles(identitiesDirectory, "*.json").Order(StringComparer.Ordinal))
            {
                ...
                try
                {
                    ...
                    var kind = ReadIdentityKind(json);
                    var daemonState = ReadString(json, "runnerDaemonState");
                    var desiredCapacity = ReadNullableInt32(json, "runnerDesiredMaxParallelism");
                    var effectiveCapacity = ReadNullableInt32(json, "runnerEffectiveMaxParallelism");
                    if (kind is not ("agent-instance" or "agentinstance" or "service" or "retired")
                        && string.IsNullOrWhiteSpace(daemonState)
                        && desiredCapacity is null
                        && effectiveCapacity is null)
                        continue;
                    identities[id] = new LegacyRunnerImport(
                        id,
                        ReadString(json, "displayName") ?? id,
                        id,
                        $"legacy-{id}",
                        string.Empty,
                        kind == "retired" ? "retired" : "migration-frozen",
        ...
        {
            if (!roles.TryGetValue(entry.Lease.ExecutorId, out var assigned))
                roles[entry.Lease.ExecutorId] = assigned = new HashSet<string>(StringComparer.Ordinal);
            assigned.Add(entry.Role);
        }
        var mixedRole = roles.FirstOrDefault(pair => pair.Value.Count > 1);
        if (!string.IsNullOrEmpty(mixedRole.Key))
            throw new InvalidDataException(
                $"Legacy executor '{mixedRole.Key}' holds both coding and review authority. Split the identities before cutover.");

        foreach (var runnerId in identities.Keys.Concat(roles.Keys).Distinct(StringComparer.OrdinalIgnoreCase).ToArray())
        {
            var latestLease = leases
            ...
            identities[runnerId] = new LegacyRunnerImport(
                runnerId,
                identity?.Name ?? runnerId,
                latestLease?.HostId ?? identity?.HostId ?? runnerId,
                latestLease?.InstanceId ?? identity?.InstanceId ?? $"legacy-{runnerId}",
                roles.TryGetValue(runnerId, out var assigned) ? assigned.Single() : string.Empty,
                identity?.Status ?? "migration-frozen",
                identity?.RegisteredAt ?? latestLease?.AcquiredAt ?? DateTime.UnixEpoch,
                identity?.LastSeenAt ?? latestLease?.ExpiresAt ?? DateTime.UnixEpoch,
                identity?.DesiredMaxParallelism,
                identity?.EffectiveMaxParallelism);
        }
        return identities.Values.OrderBy(identity => identity.RunnerId, StringComparer.Ordinal).ToArray();
    }
    ...
    private static string ReadIdentityKind(JsonElement element)
    {
        ...
        if (value.ValueKind == JsonValueKind.Number && value.TryGetInt32(out var numeric))
            return numeric switch { 1 => "agent-instance", 2 => "external-tool", 3 => "service", 4 => "retired", _ => "human" };
        return "human";
    }
