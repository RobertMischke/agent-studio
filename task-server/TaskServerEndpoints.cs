...
public static class TaskServerEndpoints
{
    public static void MapTaskServerEndpoints(this WebApplication app)
    {
        ...
        orchestratorContexts.MapPut("/projects/{projectIdentity}/dossiers/{dossierId}", async (
            HttpContext context,
            string projectIdentity,
            string dossierId,
            OrchestratorDossierContext request,
            TaskServerStore store,
            CancellationToken ct)
            => await InvokeAsync(() => store.EnsureOrchestratorContextAsync(
                projectIdentity, null, Actor(context), ct, request with { DossierId = dossierId })));
        ...
        orchestratorContexts.MapGet("/projects/{projectIdentity}/dossiers/{dossierId}/turns", async (
            HttpContext context,
            string projectIdentity,
            string dossierId,
            string? dossierKey,
            ...
            => await InvokeAsync(() => store.ReadOrchestratorContextAsync(
                projectIdentity, null, limit ?? 500, Actor(context), ct,
                DossierContext(dossierId, dossierKey, dossierTitle, lifecycleState))));
        ...
        orchestratorContexts.MapPost("/projects/{projectIdentity}/dossiers/{dossierId}/turns", async (
            HttpContext context,
            string projectIdentity,
            string dossierId,
            string? dossierKey,
            ...
            CancellationToken ct)
            => await InvokeAsync(() => store.AppendOrchestratorContextTurnAsync(
                projectIdentity, null, request, Actor(context), ct,
                DossierContext(dossierId, dossierKey, dossierTitle, lifecycleState)),
        ...
        static OrchestratorDossierContext DossierContext(
            string dossierId,
            string? dossierKey,
            string? dossierTitle,
            string? lifecycleState)
            => new(
                dossierId,
                dossierKey,
                string.IsNullOrWhiteSpace(dossierTitle) ? dossierKey ?? dossierId : dossierTitle,
                string.IsNullOrWhiteSpace(lifecycleState) ? "unknown" : lifecycleState);
