...
public sealed class ArchitectureBoundaryTests
{
    ...
    public void Windows_package_is_self_contained_and_runs_under_a_supervised_s4u_task()
    {
        ...
        Assert.Contains("icacls.exe $DataDirectory", installer);
        Assert.Contains("TaskServer", installer);
        Assert.Contains("BaseUrl", installer);

        var dryRun = File.ReadAllText(Path.Combine(
            root,
            "deploy",
            "windows",
            "task-server",
            "test-legacy-migration-dry-run.ps1"));
        Assert.Contains("requireAttemptAuthority = $true", dryRun);
        Assert.Contains("'codingAttempts'", dryRun);
        Assert.Contains("'reviewAttempts'", dryRun);
        Assert.Contains("'authorityEpoch'", dryRun);
        Assert.Contains("Get-ChildItem -LiteralPath $legacy -Force | Copy-Item", dryRun);
    }
}
