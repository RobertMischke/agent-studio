        ...
        await TempDirectory.RunAsync(async data =>
        {
            await TempDirectory.RunAsync(async legacy =>
            {
                ...
                await File.WriteAllTextAsync(Path.Combine(taskDirectory, "task.json"), """
                    ...
                    """);
                await File.WriteAllTextAsync(Path.Combine(legacy.Path, "identities", "runner-standby.json"), """
                    {
                      "id": "runner-standby",
                      "displayName": "Standby coding runner",
                      "kind": "service",
                      "registeredAt": "2026-07-16T09:00:00Z",
                      "lastSeenAt": "2026-07-17T09:59:00Z",
                      "runnerDaemonState": "ready",
                      "runnerDesiredMaxParallelism": 3,
                      "runnerEffectiveMaxParallelism": 2
                    }
                    """);
                ...
                await using (var connection = new SqliteConnection($"Data Source={store.DatabasePath}"))
                {
                    await connection.OpenAsync();
                    await using var command = connection.CreateCommand();
                    command.CommandText = "SELECT name, status, effective_max_parallelism FROM runners WHERE id = 'runner-standby';";
                    await using var reader = await command.ExecuteReaderAsync();
                    Assert.True(await reader.ReadAsync());
                    Assert.Equal("Standby coding runner", reader.GetString(0));
                    Assert.Equal("migration-frozen", reader.GetString(1));
