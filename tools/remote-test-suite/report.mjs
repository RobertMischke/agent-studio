...
function validateRun(run, index, runIds, errors) {
  ...
  if (!/^[0-9a-f]{40}$/.test(run.baseSha ?? '')) {
    errors.push(`${at}.baseSha: expected a full lowercase SHA`);
  }
  if (run.resultSha !== null && !/^[0-9a-f]{40}$/.test(run.resultSha ?? '')) {
    errors.push(`${at}.resultSha: expected a full lowercase SHA or null for expected non-delivery`);
  }
      ...
      <div class="evidence-grid">
        <section class="data-panel" aria-labelledby="sha-${anchor(run.runId)}">
          ...
          <dl class="sha-list">
            ...
            <div><dt>Result SHA</dt><dd>${run.resultSha === null
              ? '<span id="result-ref-' + anchor(run.runId) + '">Not published (expected non-delivery)</span>'
              : `<a id="result-ref-${anchor(run.runId)}" href="${escapeAttribute(`${run.rawArtifactHref}#resultSha`)}"><code>${escapeHtml(run.resultSha)}</code></a>`}</dd></div>
          </dl>
        </section>
