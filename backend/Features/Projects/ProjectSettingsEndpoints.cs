...
public static class ProjectSettingsEndpoints
{
    public static void MapProjectSettingsEndpoints(this WebApplication app)
    {
        ...
        // PUT declares or edits the build profile. First declaration closes the
        // gate. An edit preserves prior green provenance with revalidation pending;
        // the next green remote review refreshes it without starving the queue.
        ...
        app.MapPost("/api/projects/{projectName}/build-profile/validate", async (string projectName, ProjectSettingsService settings, TaskScannerService scanner, BuildProfileValidationService validator, CancellationToken ct) =>
        {
            ...
            var workingDirectory = BuildProfileValidationWorkspace.Resolve(entry);
            if (workingDirectory is null)
                return Results.BadRequest(new { error = "project source workspace is not configured or does not exist" });
