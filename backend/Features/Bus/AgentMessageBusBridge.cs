...
public sealed class AgentMessageBusBridge
{
    ...
    /// <summary>
    /// Records one terminal host-provider sign-in outcome. The device code and
    /// provider credential are deliberately absent from this contract.
    /// </summary>
        ...
        CancellationToken ct = default)
    {
        var msg = NewMessage(
            ...
            tags: new[] { "provider-sign-in", $"provider:{SafeKebab(provider)}", $"outcome:{SafeKebab(outcome)}" });
        return EmitAsync(msg, ct);
    }
