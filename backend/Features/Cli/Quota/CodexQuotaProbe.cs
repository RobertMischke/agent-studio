...
{
    // Older Codex versions rendered the reset on the limit row. Codex 0.149.0
    // wraps it onto the next bordered row, and the standard 5h row can be
    // absent while a usage-limit reset is available. The expression therefore
    // permits either 5h spelling and arbitrary non-parenthesis row chrome
    // between "left" and "(resets ...)".
    private static readonly Regex FiveHourRegex = new(
        @"5\s*h(?:our)?\s*limit\s*:?\s*(?:\[[^\]]*\]\s*)?(?<left>\d+)\s*%\s*(?:left|remaining)[^()]*\(\s*resets\s*(?<reset>[^)]+?)\s*\)",
        RegexOptions.Compiled | RegexOptions.IgnoreCase);

    private static readonly Regex WeeklyRegex = new(
        @"Weekly\s*limit\s*:?\s*(?:\[[^\]]*\]\s*)?(?<left>\d+)\s*%\s*(?:left|remaining)[^()]*\(\s*resets\s*(?<reset>[^)]+?)\s*\)",
        RegexOptions.Compiled | RegexOptions.IgnoreCase);
    ...
    private static readonly Regex FooterRegex = new(
        @"5\s*h\s*(?<h5left>\d+)\s*%\s*(?:left|remaining)?\s*[·•|]\s*weekly\s*(?<wkleft>\d+)\s*%",
        RegexOptions.Compiled | RegexOptions.IgnoreCase);
    ...
    public override async Task<QuotaSnapshot> ProbeAsync(CancellationToken ct)
    {
        ...
        try
        {
            var updatePattern  = new Regex(@"Update\s*available|Update\s*now.*Skip", RegexOptions.IgnoreCase);
            ...
            var statusPattern  = new Regex(
                @"5\s*h(?:our)?\s*limit|Weekly\s*limit|Account\s*:|codex/settings/usage|rate\s*limits\s*and\s*credits",
                RegexOptions.IgnoreCase);

            var snap = await ProbeWithStepsAsync(
            [
                // Codex 0.149.0 can put a self-update picker in front of the
                // ready prompt. Select "Skip" only when that picker is present.
                new ProbeStep("dismiss-update", WaitForPattern: updatePattern, WaitTimeoutMs: 2500, SendKeys: "<Down><Enter>", SettleIdleMs: 800, SettleTimeoutMs: 4000, SendKeysOnlyIfMatched: true),
                ...
                new ProbeStep("await-trust",   WaitForPattern: trustPattern,   WaitTimeoutMs: 3000, SendKeys: "<Enter>", SettleTimeoutMs: 5000, PreSendDelayMs: 300, SendKeysOnlyIfMatched: true),
                new ProbeStep("await-welcome", WaitForPattern: welcomePattern, WaitTimeoutMs: 5000, SendKeys: "/status", SettleIdleMs: 800, SettleTimeoutMs: 3000, PreSendDelayMs: 500),
                ...
                new ProbeStep("await-status",  WaitForPattern: statusPattern,  WaitTimeoutMs: 8000, SettleIdleMs: 1500, SettleTimeoutMs: 5000)
            ],
            initialIdleMs: 8000,
            ct);
        ...
        catch (Exception ex)
        {
            ...
            var detail = ex is OperationCanceledException
                ? "Codex /status quota probe timed out or was canceled."
                : ex.Message;
            return new QuotaSnapshot { CliType = CliType, CliVersion = cliVersion, Source = "/status", Error = detail };
        }
    }
