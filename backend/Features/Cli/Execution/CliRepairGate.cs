...
public static class CliRepairGate
{
        ...
        CancellationToken ct)
    {
        bool begin;
        HealOutcome? last;
        DateTime? previousAttemptUtc;
        lock (Lock)
        {
            var state = StateFor(cli);
            previousAttemptUtc = state.LastAttemptUtc;
        ...
        try
        {
            ...
            // NpmShimHealer folds a cancelled probe into a failed HealOutcome
            // instead of throwing, so a backend shutdown mid-repair arrives
            // here as an ordinary failure. Journaling it would write a
            // "repair FAILED" row and raise the executive-summary alarm that
            // is reserved for a genuinely broken install. Release the cooldown
            // slot this call claimed as well - a shutdown must not burn this
            // process's one attempt for the next hour - and propagate the cancel.
            if (!outcome.Available && ct.IsCancellationRequested)
            {
                lock (Lock) { StateFor(cli).LastAttemptUtc = previousAttemptUtc; }
