...
///
/// <para>
/// State is process-global and not keyed per call site or per project: the
/// npm global bin directory this repairs is one physical location per host,
/// so two concurrent job spawns racing the same broken install must share
/// one cooldown, not get one each.
/// </para>
/// </summary>
public static class CliRepairGate
{
    ...
    private static HealOutcome? _lastOutcome;
        ...
        CancellationToken ct)
    {
        ...
        lock (Lock)
        {
            begin = CliRepairCooldownPolicy.Decide(_lastAttemptUtc, nowUtc, CliRepairCooldownPolicy.DefaultWindow)
                == CliRepairCooldownDecision.Allowed;
            if (begin) _lastAttemptUtc = nowUtc;
            last = _lastOutcome;
        }
        ...
        lock (Lock) { _lastOutcome = outcome; }
    ...
    {
        lock (Lock) { _lastAttemptUtc = null; _lastOutcome = null; }
    }
}
