...
public sealed class LocalCliSelfHealService
{
    ...
    private async Task<(string Prefix, string Root)> ResolveNpmLayoutAsync(CancellationToken ct)
    {
        ...
        var prefix = prefixResult.ExitCode == 0 ? LastPathLine(prefixResult.Output) : null;
        if (string.IsNullOrWhiteSpace(prefix))
        {
            var appData = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData);
            prefix = Path.Combine(appData, "npm");
        }
        var root = rootResult.ExitCode == 0 ? LastPathLine(rootResult.Output) : null;
    ...
    private static string? LastPathLine(string? text)
    {
        var lines = text?.Replace("\r\n", "\n")
            ...
            .ToArray();
        return lines?.LastOrDefault(Path.IsPathRooted) ?? lines?.LastOrDefault();
    }
