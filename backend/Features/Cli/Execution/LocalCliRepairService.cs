...
/// package still exists but its global command shims have disappeared.
        ...
        CancellationToken ct)
    {
        ...
        try
        {
            ...
            catch (OperationCanceledException) when (!ct.IsCancellationRequested)
            {
                ...
                return new NpmInstallResult(-1, await stdoutTask.ConfigureAwait(false), await stderrTask.ConfigureAwait(false), "npm install timed out after five minutes");
            }
            ...
            foreach (var line in File.ReadLines(JournalPath).TakeLast(200))
            {
                ...
                _latest[record.CliType] = new LocalCliRepairStatus(
                    ...
                    record.Outcome == "repaired"
                        ? $"CLI repaired at {record.OccurredAt.ToLocalTime():g}"
                        : $"CLI repair failed at {record.OccurredAt.ToLocalTime():g}: {record.Error}");
            }
        }
        catch (Exception ex)
        {
        ...
        DateTimeOffset observedAt)
    {
        ...
        try
        {
            var recent = Directory.GetFiles(npmLogs, "*-debug-*.log")
                ...
                .Where(file => observedAt - file.LastWriteTimeUtc <= TimeSpan.FromHours(24))
