...
public static class DossierImplementationContract
{
        ...
        string taskKey)
    {
        ...
        if (!idempotent && !ContainsOnlyOneImplementationEntry(candidateLog))
            return Failed("One delivery may append only its own implementation entry and whitespace.");
    ...
    private static bool ContainsOnlyOneImplementationEntry(string html)
    {
        var matches = ListItemRegex.Matches(html);
        if (matches.Count != 1) return false;
        var match = matches[0];
        var attributes = AttributeRegex.Matches(match.Groups["attrs"].Value)
            .Select(attribute => attribute.Groups["name"].Value);
        if (!attributes.Contains("data-implementation-entry", StringComparer.OrdinalIgnoreCase))
            return false;

        return string.IsNullOrWhiteSpace(html[..match.Index])
            && string.IsNullOrWhiteSpace(html[(match.Index + match.Length)..]);
    }
        ...
        out string? error)
    {
        ...
        if (html.IndexOf(LogStartMarker, logStart + LogStartMarker.Length, StringComparison.Ordinal) >= 0
            || html.IndexOf(LogEndMarker, logEnd + LogEndMarker.Length, StringComparison.Ordinal) >= 0)
        {
            error = "The Dossier contains duplicate implementation log markers.";
            return false;
        }
