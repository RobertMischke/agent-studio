...
/// <c>dossier:&lt;PROJ-ID&gt;/&lt;DOSSIER-KEY&gt;</c>.
///
/// <para>
...
public sealed class OrchestratorContextKey : IEquatable<OrchestratorContextKey>
{
    ...
    /// <summary>A global, project, task, or dossier context kind.</summary>
    ...
    /// <summary>Dossier key for dossier contexts; null otherwise.</summary>
    public string? DossierKey { get; }
    ...
    public static bool TryParse(string? raw, [NotNullWhen(true)] out OrchestratorContextKey? key)
    {
        ...
        if (raw.StartsWith(dossierPrefix, StringComparison.Ordinal))
        {
            ...
            if (slash < 0) return false;
            var projectId = rest[..slash];
            var dossierKey = rest[(slash + 1)..];
            if (!IsValidIdPart(projectId) || !IsValidIdPart(dossierKey)) return false;
            key = new OrchestratorContextKey(raw, DossierKind, projectId, null, dossierKey);
            return true;
        }
