...
{
    public static readonly TimeSpan UnknownResetRetry = ProviderAccessClassifier.UnknownLimitRetry;
        ...
        TimeZoneInfo? localZone = null)
    {
        ...
        var evidence = ProviderAccessClassifier.Classify(
            ...
            text,
            null,
            observedAt: new DateTimeOffset(observedAt),
            localZone: localZone);
        if (evidence.Kind != ProviderAccessEvidenceKind.RateLimited)
            return null;

        var limitedUntil = evidence.LimitedUntil?.UtcDateTime ?? observedAt.Add(UnknownResetRetry);
