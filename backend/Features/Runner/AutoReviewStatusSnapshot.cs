...
public sealed class AutoReviewStatusSnapshot
{
    ...
    public AutoReviewStatusView Read(
        IReadOnlyList<AutoReviewActivityView>? durableActivities = null)
    {
        lock (_lock)
        {
            ...
            // A process-local worker knows a more precise step than the durable
            // leased-attempt fallback, so it wins when both describe one card.
