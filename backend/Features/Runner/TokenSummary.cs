...
public class TokenSummaryService
{
        ...
        TokenSummaryCacheStore? cache = null)
    {
        ...
        foreach (var (name, summary) in projectSummaries)
        {
            ...
            foreach (var m in summary.ByModel)
            {
                ...
                if (m.FirstRecordedAt is DateTime first
                    && (bucket.FirstRecordedAt is null || first < bucket.FirstRecordedAt))
                    bucket.FirstRecordedAt = first;
                if (m.LastRecordedAt is DateTime last
                    && (bucket.LastRecordedAt is null || last > bucket.LastRecordedAt))
                    bucket.LastRecordedAt = last;
            }
        }
    ...
    public static TokenSummary Summarize(string projectName, IReadOnlyList<OrchestratorLogEntry> entries)
    {
        ...
        foreach (var entry in entries)
        {
            ...
            if (bucket.FirstRecordedAt is null || entry.Ts < bucket.FirstRecordedAt)
                bucket.FirstRecordedAt = entry.Ts;
            if (bucket.LastRecordedAt is null || entry.Ts > bucket.LastRecordedAt)
                bucket.LastRecordedAt = entry.Ts;
        }
