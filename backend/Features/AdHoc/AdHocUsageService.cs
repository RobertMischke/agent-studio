...
public sealed class AdHocUsageService
{
        ...
        var modelList = byModel
            ...
            .Select(kv =>
            {
                ...
                return new AdHocUsageByModel(
                    ...
                    EstimatedApiCostUsd: kv.Value.Cost,
                    ModelPriced: priced,
                    FirstActivity: kv.Value.FirstActivity,
                    LastActivity: kv.Value.LastActivity);
            })
            .ToList();
    ...
    private static void Add(Dictionary<string, Bucket> map, string key, AdHocUsageRecord r, TokenCostEstimate cost)
    {
        ...
        b.Cost += cost.Total;
        var timestamp = r.Ts.ToUniversalTime();
        if (b.FirstActivity is null || timestamp < b.FirstActivity) b.FirstActivity = timestamp;
        if (b.LastActivity is null || timestamp > b.LastActivity) b.LastActivity = timestamp;
    }
