...
/// project's own thematic <c>docs/&lt;theme&gt;/common-problems</c> library without
/// an LLM call. The signal is routed to a theme (system for task-server / API /
/// project-identity / git signals, quality for frontend / UI / test signals,
/// operations for everything else, which is also the fallback inbox so a
/// misclassification stays visible on the wiki-pulse dashboard).
/// </summary>
        ...
        DateTime? nowUtc = null)
    {
        ...
        try
        {
            ...
            _logger.LogInformation(
                "Wiki maintenance {Verdict} {Project}/{JobId} slug={Slug} theme={Theme}",
                existed ? "updated" : "created", entry.Name, task.Id, signal.Slug, theme);
        ...
        catch (Exception ex)
        {
            ...
            return new WikiMaintenanceResult(WikiMaintenanceVerdict.Error, ex.Message, signal.Slug, relPath);
        }
    }
...
""";
    ...
    private static void RegenerateIndex(string commonProblemsRoot, DateTime now)
    {
        ...
        sb.AppendLine();
        sb.AppendLine("Auto-generated by the `post-wiki-maintenance` pipeline step. Do not edit manually.");
        sb.AppendLine();
    ...
    /// <summary>
    /// Routes a detected problem signal to one of the five documentation themes.
    /// Task-server / API / project-identity / git / workspace signals are the
    /// system-of-record (system); frontend / UI / test signals are quality;
    /// everything else - runner, pipeline, sweeps, CLI, permission, filesystem,
    /// state-machine - lands in operations, which is also the deliberate fallback
    /// inbox so a misrouted entry stays visible on the wiki-pulse dashboard.
    /// </summary>
    private static string ThemeFor(WikiProblemSignal signal)
    {
        var hay = (signal.Slug + " " + signal.Category + " " + string.Join(" ", signal.Tags))
            .ToLowerInvariant();
        if (hay.Contains("api") || hay.Contains("task-server") || hay.Contains("registry")
            || hay.Contains("watchpath") || hay.Contains("workspace") || hay.Contains("project-name")
            || hay.Contains("git") || hay.Contains("diff") || hay.Contains("config")
            || hay.Contains("default"))
            return "system";
        if (hay.Contains("frontend") || hay.Contains(" ui") || hay.StartsWith("ui")
            || hay.Contains("css") || hay.Contains("scss") || hay.Contains("test")
            || hay.Contains("playwright") || hay.Contains("visual"))
            return "quality";
        return "operations";
    }
