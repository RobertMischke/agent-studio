...
export class TaskStatusCardComponent {
  ...
  readonly lane = computed(() => lanePresentation(this.job().state));
  readonly laneLabel = computed(() => this.lane()?.shortName ?? this.job().state);
  readonly laneTone = computed(() => laneTone(this.job().state));

  /** Concept-doc topic for the lane-info modal, or null when none exists. */
  readonly laneTopic = computed(() => this.lane()?.docTopic ?? null);
