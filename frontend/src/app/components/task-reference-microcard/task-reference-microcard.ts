...
export class TaskReferenceMicrocardComponent {
  ...
  readonly laneIcon = computed(() => lanePresentation(this.status().lane)?.glyph ?? '◇');
  readonly laneLabel = computed(() => lanePresentation(this.status().lane)?.displayName ?? 'Deleted or unknown task');
  readonly laneToneToken = computed(() => lanePresentation(this.status().lane)?.toneToken ?? null);
