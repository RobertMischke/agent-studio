...
export class WorkbenchViewerComponent {
  ...
  private loadDocument(project: string, id: string, clear = true): void {
    ...
    this.docs.getWorkbench(project, id).subscribe({
      ...
      error: (error) => {
        if (generation !== this.requestGeneration) return;
        this.error.set(workbenchLoadError(error));
        this.loading.set(false);
      },
    });
  }
...
function workbenchLoadError(error: unknown): string {
  const candidate = error as {
    error?: { error?: unknown; message?: unknown; detail?: unknown } | string;
    message?: unknown;
  } | null;
  const payload = candidate?.error;
  const reason = typeof payload === 'string'
    ? payload
    : [payload?.error, payload?.message, payload?.detail, candidate?.message]
