...
export const CREATE_LANE_OPTIONS: readonly LaneOption[] = [TaskState.Backlog, TaskState.Preparation, TaskState.Ready]
  .map((state) => ({ state, label: LANE_PRESENTATIONS[state].shortName, icon: LANE_PRESENTATIONS[state].glyph }));
