...
- The composer disables library image attachments and uses the native context-attachment row for removable stable references. The host owns the Add context picker, while the library footer owns the model selector and Send action. No upper composer toolbar is configured.
