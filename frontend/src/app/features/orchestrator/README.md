...
- The header affordance is a chat-history switcher only. Global, project, and task rows select stored conversations or navigate to their source. Pin, refresh, one-shot include/exclude, debug, and settings belong to the compact Next message context panel above the composer. That panel names the observed view, selected history, and context included with the next message separately while reusing the same context-keyed store.
