...
- The composer disables library image attachments and uses the native context-attachment row for removable stable references. One projected Plus action opens a compact menu containing only Add context; that action opens the host-owned picker. The library footer owns the model selector and Send action. No upper composer toolbar is configured.
