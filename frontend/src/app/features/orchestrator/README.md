...
- The composer disables library image attachments and uses the native context-attachment row for removable stable references. The host owns the Add context picker, while the library footer stays limited to Add context, the model selector, and Send. Active-tab context still drives scope and picker suggestions but is not repeated as persistent footer copy. No upper composer toolbar is configured.
