...
describe('OrchestratorSideSheetComponent context attachments', () => {
  ...
  it('uses CAC context chips without a toolbar or image affordance', async () => {
    ...
    expect(root.querySelector('[data-testid="orch-composer-plus"]')).not.toBeNull();
    ...
    root.querySelector<HTMLButtonElement>('[data-testid="orch-composer-plus"]')!.click();
    fixture.detectChanges();
    const documentRoot = root.ownerDocument;
    expect(documentRoot.querySelector('[data-testid="orch-composer-menu-item-add-context"]')).not.toBeNull();
    expect(documentRoot.body.textContent).not.toContain('Upload from computer');
    expect(documentRoot.body.textContent).not.toContain('Browse the web');

    documentRoot.querySelector<HTMLButtonElement>('[data-testid="orch-composer-menu-item-add-context"]')!.click();
    fixture.detectChanges();
  ...
  it('adds, removes, and submits a free repository reference through the context envelope', async () => {
    ...
    expect(component.cacContextAttachments()).toEqual([{
      id: source.id,
      label: source.label,
      hint: 'Repository file · about 700 tokens',
    }]);
    expect(component.contextAttachmentSummaryCss()).toBe('"1 source · ~700 tokens"');
