...
describe('OrchestratorSideSheetComponent context attachments', () => {
  ...
  it('uses CAC context chips without a toolbar or image affordance', async () => {
    ...
    expect(root.querySelector('[data-testid="orch-composer-actions-trigger"]')).not.toBeNull();
    ...
    root.querySelector<HTMLButtonElement>('[data-testid="orch-composer-actions-trigger"]')!.click();
    fixture.detectChanges();
    const menu = root.querySelector('[data-testid="orch-composer-actions-menu"]');
    expect(menu?.textContent).toContain('Add context');
    expect(menu?.textContent).not.toContain('Upload');
    expect(menu?.textContent).not.toContain('Browse');

    root.querySelector<HTMLButtonElement>('[data-testid="orch-composer-action-add-context"]')!.click();
    fixture.detectChanges();
  ...
  it('adds, removes, and submits a free repository reference through the context envelope', async () => {
    ...
    expect(component.cacContextAttachments()).toEqual([{
      id: source.id,
      label: source.key,
      hint: `${source.label} · ${source.detail}`,
    }]);
    expect((fixture.nativeElement as HTMLElement)
      .querySelector('[data-testid="chat-context-attachments"]')?.textContent)
      .toContain('context-proof.md');

    (fixture.nativeElement as HTMLElement)
      .querySelector<HTMLButtonElement>('[aria-label="Remove context-proof.md from context"]')!
      .click();
    fixture.detectChanges();
