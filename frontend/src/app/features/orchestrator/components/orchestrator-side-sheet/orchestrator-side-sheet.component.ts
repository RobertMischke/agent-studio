...
export class OrchestratorSideSheetComponent implements OnInit, OnDestroy {
  ...
  /**
   * Canonical active-tab context, derived by Studio and used by the panel
   * header, the context picker recommendation, and the request envelope.
   */
  ...
  readonly composerPlusMenuOpen = signal(false);
  readonly composerPlusMenuItems: readonly MenuItem[] = [
  ...
  readonly cacContextAttachments = computed<readonly ChatContextAttachment[]>(() =>
    this.contextAttachments().map(item => ({
      ...
      hint: `${item.detail} · about ${new Intl.NumberFormat('en-US').format(item.estimateTokens)} tokens`,
    })));
  readonly contextAttachmentSummaryCss = computed(() => {
    const attachments = this.contextAttachments();
    const count = attachments.length;
    if (count === 0) return '"Context"';
    const estimatedTokens = attachments.reduce((sum, item) => sum + item.estimateTokens, 0);
    const compactTokens = new Intl.NumberFormat('en-US', {
      notation: 'compact',
      maximumFractionDigits: 1,
    }).format(estimatedTokens).toLocaleLowerCase();
    return `"${count} ${count === 1 ? 'source' : 'sources'} · ~${compactTokens} tokens"`;
  });
  ...
  readonly currentTabSource = computed<OrchestratorContextSourceOption | null>(() => {
    ...
    const locationKey = this.composerContext()?.referenceKey?.trim();
  ...
  hide(): void {
    ...
    this.composerPlusMenuOpen.set(false);
    this.contextPicker()?.close();
    this.open.set(false);
  }
  ...
  toggleComposerPlusMenu(): void {
    this.composerPlusMenuOpen.update(open => !open);
  }

  onComposerPlusMenuItem(event: MenuItemClickEvent): void {
    if (event.id === 'add-context') this.requestContextAttachment();
  }
