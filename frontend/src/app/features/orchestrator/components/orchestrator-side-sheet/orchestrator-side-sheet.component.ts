...
export class OrchestratorSideSheetComponent implements OnInit, OnDestroy {
  ...
  private readonly composerActions = viewChild<ElementRef<HTMLElement>>('composerActions');
  readonly composerActionsOpen = signal(false);
  ...
  readonly cacContextAttachments = computed<readonly ChatContextAttachment[]>(() =>
    this.contextAttachments().map(item => ({
      ...
      hint: item.key?.trim() ? `${item.label} · ${item.detail}` : item.detail,
    })));
  ...
  readonly cacComposerContext = computed<ChatComposerContext | null>(() => {
    ...
    return {
      ...
      detail: context.referenceKey?.trim() || context.detail || '',
    };
  });
  ...
  constructor() {
    ...
    effect(() => {
      ...
      untracked(() => {
        ...
        this.composerActionsOpen.set(false);
  ...
  toggleComposerActions(): void {
    if (this.sending()) return;
    this.composerActionsOpen.update(open => !open);
  }

  @HostListener('document:pointerdown', ['$event'])
  closeComposerActionsOnOutsidePointer(event: PointerEvent): void {
    if (!this.composerActionsOpen()) return;
    const target = event.target;
    if (target instanceof Node && this.composerActions()?.nativeElement.contains(target)) return;
    this.composerActionsOpen.set(false);
  }

  @HostListener('document:keydown.escape')
  closeComposerActionsOnEscape(): void {
    this.composerActionsOpen.set(false);
  }
