...
export interface DossierContextIdentity {
  dossierId: string | null;
...
type DossierIdentitySource = Partial<DossierContextIdentity> & {
  referenceKey?: string | null;
  detail?: string | null;
};

export function dossierContextIdentity(
  source: DossierIdentitySource | null | undefined,
  pageTitle: string | null = null,
): DossierContextIdentity {
  return {
    dossierId: source?.dossierId ?? null,
    dossierKey: source?.dossierKey ?? source?.referenceKey ?? null,
    dossierTitle: pageTitle ?? source?.dossierTitle ?? source?.detail ?? null,
    dossierState: source?.dossierState ?? null,
  };
}

export function resolveEffectiveDossierIdentity(
  context: ParsedOrchestratorContextKey | null,
  navigation: DossierContextIdentity,
  selected: OrchestratorContextSession | null,
  useSelection: boolean,
): DossierContextIdentity {
  if (context?.kind !== 'dossier') return dossierContextIdentity(null);
  const identity = useSelection && selected?.kind === 'dossier'
    ? dossierContextIdentity(selected)
    : navigation;
  return {
    ...identity,
    dossierId: context.dossierId,
    dossierTitle: identity.dossierTitle ?? identity.dossierKey ?? context.dossierId,
  };
}
