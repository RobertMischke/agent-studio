...
export class CliUsageModalComponent {
  ...
  readonly recordedRange = computed(() => recordedUsageRange(this.modelRows()));

  readonly recordedRangeLabel = computed(() =>
    this.recordedRange()?.label ?? 'Recording dates unavailable for legacy telemetry');
