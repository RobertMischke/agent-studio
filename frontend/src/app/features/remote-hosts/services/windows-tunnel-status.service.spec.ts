...
describe('WindowsTunnelStatusService', () => {
  ...
  it('keeps the poll timer alive for a remaining panel when a second panel stops', () => {
    // The card panel and the "Set up agent host" tunnel step share this root
    // service. Closing the dialog must not freeze the card on a stale status.
    vi.useFakeTimers();
    try {
      service.start();
      httpMock.expectOne('/api/v1/management/windows-tunnel/status').flush(sample());
      service.start();
      ...
      // The last panel releasing does stop the timer.
      service.stop();
      vi.advanceTimersByTime(30_000);
      httpMock.expectNone('/api/v1/management/windows-tunnel/status');
    } finally {
      vi.useRealTimers();
    }
  });
