...
export class RemoteHostsPanelComponent implements OnInit, OnDestroy {
  ...
  readonly codexSignIn = signal<{ host: RemoteHost; auth: ProviderAuthBadge } | null>(null);
  ...
  openCodexSignIn(target: { host: RemoteHost; auth: ProviderAuthBadge }): void { this.codexSignIn.set(target); }
  closeCodexSignIn(): void { this.codexSignIn.set(null); }
  completeCodexSignIn(): void {
    this.codexSignIn.set(null);
    this.service.reload();
  }
