...
export class RemoteHostRoleRowComponent {
  ...
  readonly roleKey = computed(() => this.host().role === 'local' ? 'local' : hostExecutorRole(this.host()));
...
export function roleSlotTotal(host: RemoteHost): number | null {
  if (host.role !== 'local' && hostExecutorRole(host) === 'review' && host.roleMaxParallelism) {
