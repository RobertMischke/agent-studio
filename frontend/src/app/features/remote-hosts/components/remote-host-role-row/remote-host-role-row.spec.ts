...
describe('roleSlotTotal', () => {
  it('uses the review role RUNNER_MAX_PARALLELISM advertisement', () => {
    expect(roleSlotTotal(host('executor:review', { roleMaxParallelism: 6 }))).toBe(6);
  });

  it('uses the coding role adopted ceiling before its bootstrap value', () => {
    expect(roleSlotTotal(host('executor:coding', {
...
function host(executor: string, overrides: Partial<RemoteHost>): RemoteHost {
  return {
    id: 'runner',
    name: 'runner',
    role: 'remote',
