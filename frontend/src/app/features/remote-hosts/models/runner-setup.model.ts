...
export function buildRunnerSetupRequest(host: RemoteHost, config: RunnerSetupConfig): VisibleCliTaskRequest {
  ...
  return {
    ...
    prompt: [
      ...
      '- Finish with the installed agent-host version, service state, Task Server reachability result, Codex and Claude login identities, refreshed LastSeen, and smoke-task key/result.',
    ].join('\n'),
  };
}
