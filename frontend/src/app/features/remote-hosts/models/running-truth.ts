...
/** Sum fresh role-local ceilings. Null keeps an absent/stale plane explicit. */
export function freshExecutionPlaneCapacity(
  hosts: readonly RemoteHost[],
  role: HostExecutionPlaneRole,
  nowMs = Date.now(),
): number | null {
  const planes = hosts
    ...
    .filter((plane): plane is HostExecutionPlaneTelemetry => plane !== null);
  if (planes.length === 0
    || planes.some(plane => plane.maxParallelism === null || plane.maxParallelism <= 0)) {
    return null;
  }
  return planes.reduce((sum, plane) => sum + plane.maxParallelism!, 0);
}
