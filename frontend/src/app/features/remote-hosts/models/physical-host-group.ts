...
/**
 * Groups process identities by their advertised physical machine. Retired
 * identities are history and remain absent until the operator asks for them.
 */
export function groupPhysicalHosts(
  ...
  return [...buckets.entries()].map(([id, bucket]) => {
    ...
    // Older snapshots may not advertise hostId. Keep their established
    // operator-facing runner name while using a namespaced fallback only as
    // the internal grouping key.
