...
- **Files tab interactive HTML**: top-level `.html` / `.htm` task artifacts join the existing Markdown and aspect JSON manifest. Expanded HTML cards render through `srcdoc` with `sandbox="allow-scripts"`; `allow-same-origin` is deliberately absent, so scripts run in an opaque origin without inheriting Studio's origin or directly reading Studio DOM, cookies, or storage. Network requests still follow normal browser and CORS policy. A small `interactive, isolated` chip makes the boundary visible.
