...
- **Overview run history is card-scoped**: `<app-overview-runs>` consumes only the open task's `RunTimeline.runs`, renders every run newest-first with start time, trigger, result, duration and the CLI/model that ran it, and derives its count badge from those visible rows. Optional per-run `tokenSummary` values appear inline when recorded; project and workspace token aggregates never enter this surface. The model is read from the run's own `executionContext` (then its token rollup) so a historical run never inherits the card's current model, and the start stamp is absolute — a relative "x ago" would churn in the polled change-detection pass. `RunRecord.reason` is shown as a full-width tail only for runs that did not complete cleanly.
