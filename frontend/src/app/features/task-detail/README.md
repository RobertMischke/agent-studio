...
- **Terminal pipeline truth**: the Overview pipeline renders its step ladder only when the selected run contains recorded step evidence. A terminal task with no `pipeline-execution.json`, or only a pre-filled Pending/Planned record, shows the explicit "No step execution recorded" state and its current lane outcome instead of future-looking rows or a zero task total. A legacy Pending CORE row with a recorded verdict is reconciled to Passed or Failed. If the latest completion-loop verdict contradicts a later terminal lane, the verdict is labelled as earlier, rendered quietly, and its stale gap is replaced by a superseded-history explanation beside the current outcome.
