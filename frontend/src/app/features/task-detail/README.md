...
- **Overview run history is card-scoped**: `<app-overview-runs>` consumes only the open task's `RunTimeline.runs`, renders every run newest-first with trigger, result, and duration, and derives its count badge from those visible rows. Optional per-run `tokenSummary` values appear inline when recorded; project and workspace token aggregates never enter this surface.
