...
describe('ResultViewComponent', () => {
  ...
  it('renders one emphasized outcome plus compact, accessible stats', async () => {
    const fixture = await build(detail(HEAD_ONLY), verdict());
    const el = fixture.nativeElement as HTMLElement;
    ...
    expect(el.querySelector('[data-testid="result-case-badge"]')?.querySelector('.result__case-dot')).not.toBeNull();
    expect(el.querySelector('[data-testid="result-metric-duration"]')?.textContent).toContain('4m');
    expect(el.querySelector('[data-testid="result-metric-duration"]')?.getAttribute('aria-label')).toBe('Duration: 4m');
    expect(el.querySelector('[data-testid="result-metric-tokens"]')?.textContent).toContain('12.0k tokens');
    expect(el.querySelector('.result__metric-icon')).toBeNull();
  });
