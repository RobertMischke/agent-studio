...
describe('ResultViewComponent', () => {
  ...
  it('renders one dot-led outcome plus compact grade, duration and token metrics', async () => {
    const fixture = await build(detail(HEAD_ONLY), verdict());
    const el = fixture.nativeElement as HTMLElement;

    expect(el.querySelectorAll('[data-testid="result-case-badge"]')).toHaveLength(1);
    expect(el.querySelector('[data-testid="result-outcome-dot"]')).not.toBeNull();
    expect(el.querySelector('.result__case-glyph')).toBeNull();
  ...
  it('renders quiet files + tests stats when the header carries the metrics', async () => {
    const md = `# Status\n\n- Result: Success\n- Files: 4\n- Tests: 12 passed\n\n## Overview\n- Problem: x\n- Solution: y\n`;
    ...
    expect(el.querySelector('[data-testid="result-metric-tests"] .result__metric-icon')).toBeNull();
  });
