import type { ArtifactImageEvent, ConversationEvent, MessageEvent } from 'coding-agent-chat/core';
...
export function groupPresentedArtifacts(
  ...
  while (index < output.length) {
    ...
    const start = index;
    while (index < output.length && output[index].kind === 'artifact.image') index += 1;
...
export function artifactBlocks(events: readonly ConversationEvent[]): ConversationArtifactBlock[] {
  ...
  while (index < events.length) {
    ...
    while (index < events.length && events[index].kind === 'artifact.image') {
    ...
    blocks.push({
      ...
      startOrdinal,
      rowCount: run.length,
      artifacts: presented.map((candidate) => candidate.artifactPresentation),
    });
  }
...
export function artifactFromPath(
  ...
  const url = kind === 'image'
    ...
    : artifactViewerUrl(normalized, jobId, watchPath);
  return {
    id,
    kind,
    path: normalized,
...
function artifactViewerUrl(
  path: string,
  jobId: string,
  watchPath: string | null | undefined,
): string {
  const encodedPath = path.split('/').map(encodeURIComponent).join('/');
  const query = watchPath ? `?watchPath=${encodeURIComponent(watchPath)}` : '';
  return `/api/tasks/${encodeURIComponent(jobId)}/results/${encodedPath.replace(/^results\//, '')}${query}`;
}
