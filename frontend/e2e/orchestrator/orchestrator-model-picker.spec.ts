...
async function expectAutomaticContext(page: Page, label: string | RegExp) {
  await page.getByTestId('orch-composer-plus').click();
  await page.getByTestId('orch-composer-menu-item-add-context').click();
  await expect(page.getByTestId('orch-context-source-picker')).toBeVisible();
  await expect(page.getByTestId('orch-context-current-automatic')).toContainText(label);
...
test('full live GPT picker persists across Board and Task contexts', async ({ page }, testInfo) => {
  ...
  await expectAutomaticContext(page, 'Board');
  ...
  await expectAutomaticContext(page, 'Deck');
  await expect(page.getByTestId('cac-model-selector-trigger')).toContainText('gpt-5.6-sol');
  await page.getByTestId(`studio-tab-url-preview:${PROJECT}:preview`).click();
  await expectAutomaticContext(page, /URL preview.*preview/);
  await expect(page.getByTestId('cac-model-selector-trigger')).toContainText('gpt-5.6-sol');
