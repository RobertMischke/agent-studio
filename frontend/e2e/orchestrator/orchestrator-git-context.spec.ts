...
test('adds current diff and known Git sources, sends typed references, and inspects their receipt', async ({ page }) => {
  ...
  await page.getByTestId('orch-context-add').click();
  ...
  const search = page.getByTestId('orch-context-search');
  await search.fill('context');
  await expect(page.getByTestId('orch-context-files').getByText(FILE)).toBeVisible();
  await page.getByTestId('orch-context-files').getByRole('button', { name: 'Add file' }).click();
  const commitResults = page.getByTestId('orch-context-commits');
  ...
  await reloadedReceipt.locator('summary').click();
  await expect(page.getByTestId('orch-answer-context-source')).toHaveCount(4);
  await page.screenshot({
