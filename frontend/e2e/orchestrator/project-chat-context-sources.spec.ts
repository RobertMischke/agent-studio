...
test('project chat attaches context references, blocks images, and keeps one permanent project chat', async ({ page }) => {
  ...
  const composerMenu = page.getByTestId('orch-composer-actions-panel');
  await expect(composerMenu).toContainText('Add context');
  await expect(composerMenu).not.toContainText('Upload');
  await expect(composerMenu).not.toContainText('Browse the web');
  await setTheme(page, 'light');
  await page.screenshot({
    path: resolve(RESULTS, 'project-chat-composer-menu--light--mocked.png'),
    fullPage: false,
  });
  await setTheme(page, 'dark');
  await page.screenshot({
    path: resolve(RESULTS, 'project-chat-composer-menu--dark--mocked.png'),
    fullPage: false,
  });
  await setTheme(page, 'light');
