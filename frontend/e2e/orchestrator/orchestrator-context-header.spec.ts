 ...
 *   1. On a project route the header shows the active project chat identity
 *      (no task, Dossier, or lane).
...
test.describe('Orchestrator context header · where am I', () => {
  ...
  test('project route shows the project chat identity', async ({ page }) => {
  ...
  test('keeps Dossier history isolated, resumes it, and restores the project chat by route', async ({ page }) => {
    ...
    await page.getByTestId('orch-side-sheet').screenshot({
      path: resolve(RESULTS, 'dossier-chat-context-list-grouped--light--mocked.png'),
    });
    await page.getByTestId('orch-context-badge').click();
    await page.getByTestId('orch-side-sheet').screenshot({
