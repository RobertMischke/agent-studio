 ...
 * matching doc under docs/. The same component is wired into three panels:
 * the global orchestrator card (concept = orchestrator), the project
 * supervisor section (concept = supervisor), and the project security
 * panel (concept = audits-and-checks).
 *
 ...
 * and closes again when the user clicks elsewhere or hits Escape.
 */
...
test('orchestrator concept-help on the global orchestrator card', async ({ page }) => {
  await page.goto('/');
  await expect(page.getByTestId('global-orchestrator-card')).toBeVisible({ timeout: 10_000 });
...
test('supervisor concept-help on the project supervisor section', async ({ page }) => {
  ...
  // The supervisor section currently lives under the Overview rail body
  // for the project; switch to it and scroll the section into view.
