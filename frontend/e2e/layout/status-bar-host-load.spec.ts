...
test.describe('Status bar execution-host load companion signal', () => {
  ...
  test('Review slots explain elevated host load without a false inconsistency hint', async ({ page }) => {
    ...
    const running = page.getByTestId('status-bar-running');
    await expect(running).toContainText('review 4 active');
    await expect(running).toHaveAttribute('data-signal-correlation', 'consistent');
    await running.hover();
    await expect(page.getByTestId('cac-tooltip')).toContainText('Review plane 4 active, 0 waiting');
  ...
  test('waiting Review cards with zero active workers render as amber attention', async ({ page }) => {
    await stubHostLoad(page, 0, 0, 0, 1.2, 0, 7);
    await page.goto('/');

    const running = page.getByTestId('status-bar-running');
    await expect(running).toContainText('review 0 active / 7 waiting');
    await expect(running).toHaveAttribute('data-signal-tone', 'mismatch');
    await expect(running).toHaveAttribute('data-signal-correlation', 'review-waiting-without-active');
    await expect(page.getByTestId('status-bar-running-divergence'))
      .toHaveAttribute('aria-label', 'Review queue needs attention: waiting cards have no active reviewer.');
    await running.hover();
    await expect(page.getByTestId('cac-tooltip')).toContainText(
      'Attention: 7 Review cards are waiting with no active review.',
    );
    await expect(page.getByTestId('cac-tooltip')).toContainText(
      'Consistency hint: post-processing is waiting without an active Review worker.',
    );
