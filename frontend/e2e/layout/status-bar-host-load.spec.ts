...
test.describe('Status bar execution-host load companion signal', () => {
  ...
  test('healthy local CLIs consume no status-bar space in both themes', async ({ page }) => {
    ...
    await setTheme(page, 'light');
    await page.screenshot({
      path: join(RESULTS_DIR, 'healthy-cli-zero-status-space-light--mocked.png'),
      fullPage: false,
    });
    await setTheme(page, 'dark');
    await page.screenshot({
      path: join(RESULTS_DIR, 'healthy-cli-zero-status-space-dark--mocked.png'),
      fullPage: false,
    });
  });
