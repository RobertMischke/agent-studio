 ...
 * component itself, its responsive CSS, and both theme token sets are real.
 *
 * Build first with: npm run build:mockup:planning
 */
...
test.beforeAll(async () => {
  if (!fs.existsSync(path.join(DIST_DIR, 'index.html'))) {
    throw new Error(`Missing ${DIST_DIR}. Run "npm run build:mockup:planning" first.`);
  }
...
test.describe('@mockup planning follow-up status', () => {
  test('shows one compact message with both actions in both themes and a narrow panel', async ({ page }) => {
    ...
    await expect(panel.getByTestId('planning-contract')).toHaveCount(0);
