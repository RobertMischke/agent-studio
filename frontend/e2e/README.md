...
| `model-routing-policy.spec.ts` | Task-type model routing suggestion |
...
| Spec | Summary |
|------|---------|
| `active-board-filter-chip.spec.ts` | active board filter stays visible and clears by X, Escape, and an unfiltered route |
| `archive-all-async.spec.ts` | Async archive all |
...
| `depends-on-release-gate.spec.ts` | dependsOn release gate board status |
...
| `model-family-badge-contrast.spec.ts` | Candidate A keeps model-family badges AA-safe across Studio surfaces and preserves dark defaults |
...
| `provider-auth-wait.spec.ts` | Ready card shows the provider sign-in wait reason in both themes |
| `remote-running-card.spec.ts` | Remote lease drives the board running state |
| `runner-queue-dependency-precedence.spec.ts` | runner queue dependency precedence |
...
| `activity-structured-content.spec.ts` | Activity renders structured tool payloads and runner events quietly in ${theme} theme |
...
| `git-view-layout-shots.spec.ts` | AGT-2453 Git-View topology and layout shots (mocked) |
...
### `orchestrator/` - 20 specs

| Spec | Summary |
|------|---------|
| `f14-context-chip-and-caching.spec.ts` | F14: context badge, menu and per-message sending |
| `orchestrator-chat-content-visible.spec.ts` | orchestrator chat - content stays visible after load |
| `orchestrator-chat-history.spec.ts` | lists current Task Server contexts and opens a chat in ${theme} theme |
...
| `orchestrator-project-entry.spec.ts` | Orchestrator Chat shell posture and activity |
...
| `project-chat-context-sources.spec.ts` | project chat attaches known sources, inspects the persisted receipt, and keeps one permanent project chat |
...
| `project-cycle-time-panel.spec.ts` | cycle time rail renders aggregates, composition, drill-down, and both themes |
...
| `project-token-usage-freshness.spec.ts` | current receipt, source timestamp, and partial-data warning render in both themes |
...
| `prompt-registry-observability.spec.ts` | prompt registry reviews shipped prompts and exposes provenance, overrides, calls, and cost |
| `proposals-hub.spec.ts` | Project Hub proposals render in both themes |
| `wiki-agent-reads.spec.ts` | Wiki agent read evidence |
| `wiki-folder-git-date.spec.ts` | wiki folder marks the filesystem-time fallback for an untracked page |
| `wiki-linked-elements-navigation.spec.ts` | linked dossier anchors navigate, track scrolling, return, and expose missing targets |
...
| `workbench-details-panel.spec.ts` | Dossier details keeps a session draft and creates the feature card |
| `workbench-dossier-assets.spec.ts` | the timeline-redesign Dossier renders its assets/ screenshots instead of a broken image |
| `workbench-overview-live.spec.ts` | captures the Dossier overview at wide and narrow widths in both themes |
...
| `workbench-readonly.spec.ts` | article patterns drive the Dossier Explorer and isolated viewer in both themes |
| `workbench-viewer-header.spec.ts` | ${dossier.fixture.key} ${dossier.label} dossier fills the viewer after late load and scrolls internally |
...
| `dossier-navigation.spec.ts` | Dossier navigation focuses one catalogue path, persists disclosures, and collapses all |
...
| `tab-pin-and-quiet-close.spec.ts` | studio-shell · pinned tabs |
...
| `provider-limit-and-breaker-banner.spec.ts` | provider-limit and infra-breaker notices remain distinct and explicit |
...
| `archive-and-next-on-completed.spec.ts` | non-integrated archive warns, then archives and advances after confirmation |
...
| `concept-dossier-notice.spec.ts` | concept completion shows one compact dossier sensor and links the recorded path |
| `decision-surface.spec.ts` | operator decision surface |
...
| `detail-keyboard-containment.spec.ts` | Task detail keyboard containment |
...
| `pipeline-remote-lifecycle.spec.ts` | AGT-2427 before and after: remote lifecycle, review, gate, ledger cost and calls |
...
| `pipeline-step-usage.spec.ts` | missing historical prices never render as zero and mixed totals stay explicit |
...
| `prompt-enrichment-report.spec.ts` | keeps the MKT-17 prompt readable with compact enrichment (${viewport}, ${theme}) |
...
| `result-scaffold-presentation.spec.ts` | Result scaffold presentation |
...
| `runner-replay-simulated-badge.spec.ts` | replayed public-demo runner events are labelled Simulated in both themes |
...
| `task-detail-instant-navigation.spec.ts` | opens the task head before heavy detail data resolves |
...
| `task-header-live-status.spec.ts` | Task-header live status |
| `task-orchestrator-chat.spec.ts` | Task detail uses the Orchestrator side sheet for task context without a Chat tab |
| `task-tab-refinement-history.spec.ts` | Task inspector tab |
| `timeline-event-density.spec.ts` | Timeline ${eventFixture.kind} stays quiet, specific, and non-redundant |
...
| `explorer-lane-dots-match-board.spec.ts` | tree lane dots count exactly what the board lanes render, with an epic in the feed |
