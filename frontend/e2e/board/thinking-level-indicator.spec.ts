...
function task(id: string, title: string, configured: string, effective: string) {
  return {
    ...
    execution: {
      ...
      thinkingLevel: effective, runOutcome: 'success',
    },
  };
}
...
const tasks = [
  ...
  task('effective-level', 'Effective run level stands out', 'ultra', 'medium'),
];
const grouped = {
...
test('shows effective thinking level and highlights only a runtime mismatch in both themes', async ({ page }) => {
  ...
  await expect(levels.nth(1)).toHaveText('m');
  await expect(levels.nth(1)).toHaveAttribute('data-thinking-level', 'medium');
  ...
  await page.screenshot({
    path: 'C:/Projects/agent-taskboard-workspace/projects/agent-taskboard/tasks/002/AGT-2075/results/thinking-level-card--mocked.png',
    fullPage: true,
  });
});
