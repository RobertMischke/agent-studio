...
test.describe('Token popover exclusivity + dismissal (AGT-2675)', () => {
  ...
  // Operator clarification (2026-08-23): on the Ready lane in the screenshot the
  // panels were not popovers that failed to close — they were painted for
  // *every* card straight away, with no hover or click at all. Root cause is the
  // component's unconditional `:host { display: block }`: an author `display`
  // rule beats the UA stylesheet's `[hidden] { display: none }`, so the `hidden`
  // attribute the directive toggles never actually hid anything. Assert the
  // at-rest state on a lane the size of the one in the screenshot.
  test('no panel is visible at rest on a 30+ card lane', async ({ page }) => {
    await page.setViewportSize({ width: 1280, height: 900 });
    await seedBoardTab(page);
    const jobs = Array.from({ length: 32 }, (_, i) => makeJob(`token-rest-${i}`, i + 1, 60_000 + i * 1_000));
    await installRoutes(page, jobs);
    await page.goto('/');
    await page.waitForLoadState('domcontentloaded');

    await expect(page.locator('[data-testid="task-card"]')).toHaveCount(32, { timeout: 10_000 });

    // Screenshot before asserting, so this same spec is the evidence producer
    // in both states: run it with the fix reverted and the image shows the
    // reported stack of panels covering the lane; run it as-is and the lane is
    // clean. Persisted to the task results folder by the artifact reporter.
    await page.screenshot({ path: 'test-results/token-popover-lane-at-rest.png', fullPage: false });

    // Every card still renders its popover element (the directive portals the
    // existing template node on demand); none of them may be shown.
    const popovers = page.locator('[data-testid="task-card-token-popover"]');
    await expect(popovers).toHaveCount(32);
    await expect(page.locator('[data-testid="task-card-token-popover"]:not([hidden])')).toHaveCount(0);

    // `hidden` present is necessary but not sufficient — the regression was
    // precisely a `hidden` attribute with no rendering effect. Assert the
    // computed style actually removes every panel from layout.
    const shown = await popovers.evaluateAll((nodes) =>
      nodes.filter((n) => getComputedStyle(n).display !== 'none').length);
    expect(shown, 'no token panel may have a layout box before any interaction').toBe(0);
  });
});
