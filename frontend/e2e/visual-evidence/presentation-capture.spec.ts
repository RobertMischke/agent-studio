...
async function captureBoth(page: Page, testInfo: TestInfo, order: string, name: string): Promise<void> {
  for (const theme of ['dark', 'light'] as const satisfies readonly Theme[]) {
    await setTheme(page, theme);
    await shot(page, testInfo, `${order}-${name}--${theme}--real`);
  }
}
...
async function openBoard(page: Page): Promise<void> {
  ...
  await expect(page.locator(TASK_CARD).filter({ hasText: 'DEMO-5' }).first()).toBeVisible();
}

async function openDemoTask(page: Page): Promise<void> {
  await openBoard(page);
  await page.locator(TASK_CARD).filter({ hasText: 'DEMO-5' }).first().click();
...
test('presentation still 04 - orchestrator conversation', async ({ page }, testInfo) => {
  ...
  await expect(page.getByTestId('chat-body')).toContainText('MVP walkthrough');
