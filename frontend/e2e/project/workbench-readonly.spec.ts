...
test('Dossier Explorer and isolated viewer use real repository artifacts in both themes', async ({ page, devBackend }, testInfo) => {
  ...
  await expect(page.getByText('Dossiers / open experiment', { exact: true })).toBeVisible();
  await expect(frame).toHaveAttribute('title', /^Dossier artifact:/);
  await expect(frame).toHaveAttribute('sandbox', 'allow-scripts');
  await expect(page.getByTestId('workbench-viewer-provenance')).toContainText('docs/concepts/mockups/experimentier-workbench.html');
